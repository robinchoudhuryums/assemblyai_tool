...
- Submitted jobs are tracked via `batch-inference/active-jobs/${jobId}.json`. Post-submission tracking writes are retried 3× with exponential backoff; on persistent failure the tracking data falls back to `batch-inference/orphaned-submissions/${jobId}.json` and a Sentry `error` alert is raised with the jobId/jobArn recovery keys. The next batch cycle's `promoteOrphanedSubmissions()` scan self-heals the orphan back to `active-jobs/`.
...
| GET | `/api/dashboard/weekly-changes` | authenticated | Week-over-week narrative: top score movers, flag deltas, noteworthy calls. Backs the "This Week in Review" dashboard widget. |
...
| **PHI redaction in logs** | `server/services/phi-redactor.ts`, `shared/phi-patterns.ts` | 14 regex patterns (SSN, DOB, MRN, phone, email, address, Medicare/Medicaid IDs, names) auto-redact the `detail` field in all audit entries before persistence. `shared/phi-patterns.ts` mirrors the same patterns for client-side use (Sentry scrubbing). |
| **Client-side PHI scrubbing (Sentry)** | `client/src/lib/sentry.ts`, `shared/phi-patterns.ts` | Recursively scrubs `event.user`, `event.request`, `event.contexts`, `event.extra`, `event.tags`, breadcrumb `data` + `message`, and exception values via `deepScrubPhi` (depth=6, WeakSet cycle detection, 10KB string cap). Fail-open: a scrubber crash logs via `console.error` and the event still goes through. Sentry namespace re-export removed — callers must use `captureException` / `captureMessage` wrappers. |
...
- **Client Sentry scrubber is fail-open** (A6, Batch 1): symmetric with the server `beforeSend`. If `deepScrubPhi` crashes in `client/src/lib/sentry.ts`, the event is sent unmodified to Sentry and the failure is logged via `console.error`. Observability priority — dropping events on scrubber bugs would blind us to both the original error and the bug. Trade-off: brief PHI-in-Sentry window during a scrubber bug, bounded by `console.error` alerting.
- **`shared/phi-patterns.ts` is the single source of truth for PHI patterns** (A6, consolidated): `server/services/phi-redactor.ts` now imports all 14 regex patterns from `@shared/phi-patterns` instead of maintaining a parallel copy. Adding a new pattern requires updating only `shared/phi-patterns.ts` — both server (audit logs, logger) and client (Sentry scrubber) pick it up automatically.
...
- **Score-tier constants partial migration** (A11, Batch 2): 5 sites converted to `SCORE_EXCELLENT/GOOD/NEEDS_WORK` constants (`report-components`, `agent-scorecard` ×2, `calls-table`, `performance`); 4 still hardcode (`team-analytics`, `call-clusters`, `my-performance`, `leaderboard`). Future changes to the thresholds will propagate inconsistently until the remaining sites are converted. The static client constants mirror `server/constants.ts` and are the bundled fallback for `useConfig()`.
- **A/B test promotion is on-demand only** — `promoteActiveModel()` in `server/services/active-model.ts` updates `aiProvider` (the on-demand Bedrock singleton) and persists to S3, but does NOT update `bedrockBatchService`. Trade-off: batch-mode users keep the pre-promotion model until restart. Acceptable because batch mode is opt-in and admins who promote after a Haiku A/B test typically want the change to apply to the sync path first. Fix is scoped as a follow-on.
...
- **`BEDROCK_MODEL` is validated at startup and at runtime** — `server/index.ts` emits a `console.warn` on boot if the env var is set but missing from `BEDROCK_PRICING`. The pipeline's usage-tracking path also calls `warnOnUnknownBedrockModel()` on the first call per unknown model id (deduped via a module-level Set) and sends a Sentry `captureMessage` at "warning" level. Trade-off: the warning is non-blocking — cost records still store `$0` for unknown models. Operators who want a hard-fail must add the check themselves.
...
- **Sentry namespace export removed (server AND client)** — import `captureException` / `captureMessage` from `server/services/sentry.ts` (server) or `client/src/lib/sentry.ts` (client). Direct `Sentry.*` access bypassed PHI scrubbing and is no longer available. The client-side scrubber recursively walks `event.user`, `event.request`, `event.contexts`, `event.extra`, `event.tags`, and breadcrumb data via the patterns in `shared/phi-patterns.ts` — see the HIPAA Compliance table row.
...
- **Score tier thresholds: 5 of ~9 client-side sites converted** (A11, Batch 2) — `report-components`, `agent-scorecard` (×2), `calls-table`, `performance` use `SCORE_EXCELLENT/GOOD/NEEDS_WORK`. `team-analytics`, `call-clusters`, `my-performance`, `leaderboard` still hardcode their thresholds. Drift risk on threshold changes — update both sets in lock-step until the remaining four are converted.
...
- **`/api/calls` cache key convention** (A14, Batch 2) — omit the filter object entirely from the query key when no filters are set (use `["/api/calls"]`, not `["/api/calls", { status: "", sentiment: "", employee: "" }]`). Empty-string sentinels create cache keys that don't match the filterless invalidation pattern used by mutations (e.g. upload-complete webhook invalidation). Dashboard and sidebar follow this convention after A14; `client/src/pages/sentiment.tsx:19` still has the old anti-pattern — pending follow-up.
...
- **`aiProvider.setModel()` is asymmetric with `bedrockBatchService`** (active-model promotion) — `POST /api/ab-tests/promote` calls `aiProvider.setModel()` which swaps the model on the on-demand singleton. `bedrockBatchService` caches its model at construction from `process.env.BEDROCK_MODEL` and does NOT observe the promotion. If `BEDROCK_BATCH_MODE=true` when an admin promotes a new model, in-flight batch jobs still use the pre-promotion model until the next restart (at which point `loadActiveModelOverride()` re-applies the override on the on-demand path). Batch jobs remain on the env-var model until `bedrockBatchService` is patched to observe promotions.
- **`requireAuth` is now `async`** — the fingerprint-mismatch branch awaits both `req.logout()` and `req.session.destroy()` before responding 401 to prevent a window where a failed destroy left the cookie valid on the session store. Happy path is unaffected. Destroy failures escalate to Sentry via dynamic import. Express 4.21 handles the async middleware; downstream composers must not assume `requireAuth` returns synchronously.
- **`DELETE /api/calls/:id/tags/:tagId` enforces author-or-manager** — previously any authenticated user (including viewers) could delete any tag. The handler now does a SELECT-then-author-check-then-DELETE, mirroring the annotation delete pattern at the same file's `DELETE /api/calls/:id/annotations/:annotationId`. Only the tag's `created_by` match OR a manager/admin can delete. If a user's display name changes, previously-created tags will no longer match the author check — they can still be deleted via the manager+ fallback.
- **`advanceIncidentPhase` is DB-first via clone pattern** — builds the new phase state on a shallow clone (with a sliced timeline array), calls `persistIncident()` first, and only applies the mutation to the in-memory `incident` object on successful persist. Previously a failed persist left the in-memory cache ahead of the DB. **Follow-on**: `addIncidentTimelineEntry` and `addActionItem` in the same file still have the old mutate-then-persist anti-pattern and are susceptible to the same drift.
- **Audit queue overflow: drop-oldest + Sentry escalation** — `MAX_QUEUE_SIZE=20000` in `server/services/audit-log.ts` gives ~20MB of runway under sustained DB outage. When full, the OLDEST entry is shed from the DB write path (head of queue is most likely already toxic with failed retry attempts). The canonical non-repudiable record remains in stdout via the HMAC chain, so operators can reconstruct the missing rows from captured pm2/CloudWatch logs by walking the `[HIPAA_AUDIT]` lines. The first drop per process escalates to Sentry `captureMessage` at "error" severity — operators get paged once per outage, not per dropped entry. Subsequent drops in the same process log via `logger.error` but do not re-alert.
- **Batch inference tracking-write has retry + orphan fallback** — after `bedrockBatchService.createJob()` succeeds, the AWS job is running and billable. If the subsequent `batch-inference/active-jobs/${jobId}.json` write fails, the job becomes invisible to CallAnalyzer. `persistBatchJobTracking()` in `batch-scheduler.ts` retries 3× with exponential backoff (1s/2s/4s), then falls back to writing `batch-inference/orphaned-submissions/${jobId}.json` so the job data still lives in S3 under a known prefix. `promoteOrphanedSubmissions()` scans that prefix at the top of every batch cycle and promotes surviving entries back to `active-jobs/`, then deletes the orphan copy. In all tracking-write failure paths, a Sentry `captureMessage` at "error" fires with the jobId and jobArn — those are the recovery keys an operator uses to manually reconstruct the active-jobs file from the AWS console if even the orphan write fails.
...
| **Batch Inference** | `server/services/bedrock-batch.ts`, `server/services/batch-scheduler.ts` | Deferred AI analysis via JSONL to S3, periodic job submission/polling/recovery. `bedrock-batch.ts` uses `sigv4.ts` directly for S3 operations, resolves AWS creds lazily via `getAwsCredentials()` (env→IMDS), validates `BEDROCK_BATCH_ROLE_ARN`, and paginates `s3List` via continuation token (50-page safety cap). `batch-scheduler.ts:persistBatchJobTracking()` retries the post-createJob tracking write 3× (1s/2s/4s backoff) with a `batch-inference/orphaned-submissions/${jobId}.json` fallback on persistent failure, and `promoteOrphanedSubmissions()` runs at the top of every batch cycle to self-heal orphans. All tracking-write failures fire a Sentry `captureMessage` at "error" with the jobId + jobArn recovery keys. |
| **Scoring** | `server/services/scoring-calibration.ts`, `server/services/auto-calibration.ts`, `server/services/scoring-feedback.ts` | Score normalization, periodic distribution analysis, manager correction capture for future prompt injection. Correction `reason` text is passed through `sanitizeReasonForPrompt()` at capture AND render time and wrapped in `<<<UNTRUSTED_MANAGER_NOTES>>>` delimiters in `buildCorrectionContext()` — a prompt-injection defense that must be preserved by future edits. |
...
| **PHI Protection** | `server/services/phi-redactor.ts`, `shared/phi-patterns.ts`, `client/src/lib/sentry.ts`, `server/services/prompt-guard.ts` | 14-pattern PHI redaction — single source of truth in `shared/phi-patterns.ts`, imported by both `phi-redactor.ts` (server audit logs, logger) and `sentry.ts` (client Sentry scrubber). Server `redactPhi()` adds count tracking; client `deepScrubPhi` adds recursive object traversal. 16-pattern prompt injection detection + output anomaly scanning (`prompt-guard.ts`) |
...
- `shared/phi-patterns.ts` → consumed by `client/src/lib/sentry.ts` and `server/services/phi-redactor.ts` (consolidated — single source of truth for all 14 PHI regex patterns).
