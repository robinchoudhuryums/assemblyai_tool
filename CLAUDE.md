...
npm run test         # Run backend tests (tsx --test --test-force-exit tests/*.test.ts — 939 tests)
...
| GET | `/api/reports/filtered/export.csv` | manager+ | Server-side CSV export of `/api/reports/filtered` data (summary + sentiment + sub-scores + performers + monthly trends). Writes a first-class `event: "export_report"` HIPAA audit entry, replacing the client-fire-and-forget beacon pattern for this surface. Accepts the same `from` / `to` / `employeeId` / `role` / `callPartyType` query params. |
...
- **Shared report-export helpers in `routes/utils.ts`** — CSV: `buildCsv(sections)` (multi-section body with `escapeCsvValue` formula-injection protection) + `writeCsvResponse(res, csv, filename, audit?)`. PDF (Phase D): `buildPdfBuffer(sections, metadata)` + `writePdfResponse(res, buf, filename, audit?)`. Both accept the SAME `CsvSection[]` shape so one aggregation can feed both formats — the filtered-report, agent-profile, and coaching-outcomes endpoints ship CSV + PDF variants that reuse the same section data. `buildPdfBuffer` lazy-imports `pdfkit` so test runs and the audio-pipeline boot path don't pay the font-load cost. Add new export endpoints via these helpers rather than hand-rolled string concatenation or direct pdfkit usage.
...
| **Reports** | `server/routes/reports.ts` | Search, agent profiles, filtered reports, AI-generated agent summaries, client-export audit beacon (`POST /api/reports/export-beacon` — HIPAA), scoring-corrections self-view (`GET /api/scoring-corrections/mine`), semantic search (`GET /api/search/semantic` — pgvector cosine when available, in-memory fallback, hybrid mode `?mode=hybrid&alpha=N` fusing keyword reciprocal-rank + cosine, relevance threshold `?threshold=0.25` default, layered `from`/`to`/`sentiment`/`employeeId` filters, 200-entry LRU query-embedding cache, graceful keyword fallback), server-side CSV + PDF exports (`GET /api/reports/filtered/export.{csv,pdf}`, `GET /api/reports/agent-profile/:employeeId/export.{csv,pdf}` — all emit `event: "export_report"` audit entries). |
| **Coaching** | `server/routes/coaching.ts` | Coaching session CRUD, action item toggling, webhook triggers, per-session outcome (`GET /api/coaching/:id/outcome`), program-wide outcomes aggregate (`GET /api/coaching/outcomes-summary` — `?groupBy=manager\|employee` per-group breakdown, `?bucket=week` weekly time-series, `?includeSkipped=true` drill-in, `avgSubDeltas` sub-score deltas on every rollup), CSV + PDF exports (`/export.csv`, `/export.pdf` — both emit `event: "export_report"` audit entries). |
...
| **Webhooks** | `server/services/webhooks.ts` | HMAC-signed HTTP POST notifications on call events with retry logic and SSRF validation. Exports `setWebhookRetryEnqueuer(fn)` + `redeliverWebhook({webhookId, event, body, previousAttempts})` so the in-process `deliverWithRetry` can hand off to the durable PostgreSQL job queue after 4 in-process retries exhaust — survives process restart. Without `DATABASE_URL`, the enqueuer is never wired and behavior falls back to prior fire-and-forget. Phase C: per-webhook circuit breaker (`webhookBreaker` — 5 failures / 5 min threshold, LRU-bounded at 1k keys) wrapping `deliverWebhook`. Open circuit skips BOTH delivery AND durable-retry enqueue for that `webhookId`. Exports `getWebhookBreakerSnapshot` for the admin observability endpoint. Powers the `/admin/webhooks-health` dashboard. |
