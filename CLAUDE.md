...
npm run test         # Run backend tests (tsx --test --test-force-exit tests/*.test.ts — 898 tests)
npm run test:coverage # Backend tests with c8 coverage report (text + text-summary)
npm run test:client  # Run frontend tests (Vitest + React Testing Library — 200 tests)
...
| GET | `/api/search/semantic` | authenticated (viewer: own calls only) | Embedding-based semantic search. Embeds the query via `aiProvider.generateEmbedding`, scores stored `call_analyses.embedding` vectors via cosine similarity, returns top-N. Query: `q` (required, ≤500 chars), `limit` (default 20, max 100). Viewer-scoped via `canViewerAccessCall`. Gracefully falls back to keyword search via `mode: "keyword-fallback"` when embedding is unavailable (Bedrock unreachable or not configured). Response includes `coverage: { totalAccessible, withEmbeddings }` so clients can see how many calls had embeddings. |
...
| GET | `/api/coaching/outcomes-summary` | manager+ | Program-wide coaching effectiveness aggregate over a 90-day window (configurable via `?days`, range 7–365). Rolls up per-session before/after deltas using the same logic as `/api/coaching/:id/outcome`. Returns `{ totalSessions, measured, insufficientData, positiveCount (delta ≥+0.5), neutralCount, negativeCount (delta ≤-0.5), avgOverallDelta }`. Powers the "Program effectiveness" strip at the top of the manager coaching page. |
...
- **`embeddingCosineSimilarity` is a shared export** (`server/services/call-clustering.ts:206`) — used by both call-clustering and the new `/api/search/semantic` endpoint. Pure function, no side effects.
...
| **Reports** | `server/routes/reports.ts` | Search, agent profiles, filtered reports, AI-generated agent summaries, client-export audit beacon (`POST /api/reports/export-beacon` — HIPAA), scoring-corrections self-view (`GET /api/scoring-corrections/mine`), semantic search (`GET /api/search/semantic` — cosine similarity over stored embeddings, viewer-scoped, keyword fallback), server-side CSV export (`GET /api/reports/filtered/export.csv` — first-class `event: "export_report"` audit entry). |
| **Coaching** | `server/routes/coaching.ts` | Coaching session CRUD, action item toggling, webhook triggers, per-session outcome (`GET /api/coaching/:id/outcome`), program-wide outcomes aggregate (`GET /api/coaching/outcomes-summary` — avg delta + up/flat/down session counts over a configurable window). |
| **Users** | `server/routes/users.ts` | User management (admin CRUD, password reset/change, MFA). Also `GET /api/users/unlinked` returning viewer users with no matching employee row — backs the admin Users-tab "Onboarding" banner. |
...
| **Job Queue** | `server/services/job-queue.ts` | PostgreSQL-backed durable queue with `FOR UPDATE SKIP LOCKED`, 30s worker heartbeat, 2min stale reap, attempts-on-failJob contract (A18). `getJob(jobId)` lookup added (A8) for polling-based async job patterns. Worker handlers in `routes.ts`: `process_audio` (audio pipeline), `batch_snapshots` (snapshot generation), `deliver_webhook` (persistent webhook retry — handoff from in-process retry exhaustion). |
...
- [ ] **Viewer user accounts must be linkable to an employee row** — `server/auth.ts:getUserEmployeeId()` matches `username→email` then `displayName→name`. If no match, viewers see empty call lists and 403 on agent endpoints. Silent degradation — no startup warning. **MEDIUM risk** — viewer appears authenticated but sees no data. Now observable: `GET /api/users/unlinked` returns the list of viewer accounts with no matching employee, and the admin Users tab renders an amber-stripe "Onboarding" banner listing them so the mismatch is visible to admins during account creation. Ensure DB employee records use the same email as the corresponding user's login; or adjust the user's display name to match an existing employee's name.
...
### Test Commands
```bash
npm test                   # Backend (898 tests)
npm run test:client        # Frontend (200 tests)
npm run check              # TypeScript type check
```
