...
npm run test         # Run backend tests (tsx --test --test-force-exit tests/*.test.ts — 908 tests)
npm run test:coverage # Backend tests with c8 coverage report (text + text-summary)
npm run test:client  # Run frontend tests (Vitest + React Testing Library — 204 tests)
...
| GET | `/api/coaching/outcomes-summary` | manager+ | Program-wide coaching effectiveness aggregate over a 90-day window (configurable via `?days`, range 7–365). Optional `?groupBy=manager\|employee` returns per-group breakdown — flat response is `{ totalSessions, measured, insufficientData, positiveCount (delta ≥+0.5), neutralCount, negativeCount (delta ≤-0.5), avgOverallDelta }`; grouped response is `{ overall, groups[] }` where each group carries the same fields plus `key` + `label`. Rolls up per-session before/after deltas using the same logic as `/api/coaching/:id/outcome`. Powers the "Program effectiveness" strip + per-manager breakdown table on the manager coaching page. |
...
- **Shared CSV helpers in `routes/utils.ts`** — `buildCsv(sections)` builds a multi-section CSV body with formula-injection escaping via `escapeCsvValue`; `writeCsvResponse(res, csv, filename, audit?)` writes the response with a sanitized filename + optional audit callback. Used by all server-side report-export endpoints (filtered report, agent profile, coaching outcomes, leaderboard). Adding a new export endpoint should use these rather than hand-rolled string concatenation.
...
| **Reports** | `server/routes/reports.ts` | Search, agent profiles, filtered reports, AI-generated agent summaries, client-export audit beacon (`POST /api/reports/export-beacon` — HIPAA), scoring-corrections self-view (`GET /api/scoring-corrections/mine`), semantic search (`GET /api/search/semantic` — pgvector cosine when available, in-memory fallback, hybrid mode `?mode=hybrid&alpha=N` fusing keyword reciprocal-rank + cosine, relevance threshold `?threshold=0.25` default, layered `from`/`to`/`sentiment`/`employeeId` filters, 200-entry LRU query-embedding cache, graceful keyword fallback), server-side CSV exports (`GET /api/reports/filtered/export.csv`, `GET /api/reports/agent-profile/:employeeId/export.csv` — both emit `event: "export_report"` audit entries). |
| **Coaching** | `server/routes/coaching.ts` | Coaching session CRUD, action item toggling, webhook triggers, per-session outcome (`GET /api/coaching/:id/outcome`), program-wide outcomes aggregate (`GET /api/coaching/outcomes-summary` — avg delta + up/flat/down session counts over a configurable window; `?groupBy=manager\|employee` for per-group breakdown), CSV export (`GET /api/coaching/outcomes-summary/export.csv`). |
...
### Test Commands
```bash
npm test                   # Backend (908 tests)
npm run test:client        # Frontend (204 tests)
npm run check              # TypeScript type check
```
