...
- **`[RETENTION]` bracket prefix still exists at `server/index.ts:565`** via the `vite.ts:log()` helper — the last remaining literal bracket prefix after F-15 removed `[AUDIT]`. Same class of concern (bypasses the structured logger's PHI redaction); kept because the line is emitted only on the retention-purge path which has no PHI in its payload. Scrapers grepping for any bracket prefix except `[HIPAA_AUDIT]` match exactly this one line.
