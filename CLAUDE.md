...
- **Design system first installment lands with the Simulated Call Generator** — `client/src/pages/simulated-calls.tsx` is the first page redesigned against the new tokens. Subsequent pages (Dashboard, Transcript Viewer, Coaching, etc.) will be redesigned as their handoff bundles arrive. Legacy utilities in `index.css` (`.sentiment-positive/.negative/.neutral`, `.metric-card`, `.chart-container`, `.drop-zone-active`, `.gradient-border*`) still use pre-migration hardcoded HSL and will be revisited when their pages' designs land. Do not add new usages of those legacy utilities.
