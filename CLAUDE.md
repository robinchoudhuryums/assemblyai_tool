...
- **`shared/phi-patterns.ts` parallels `server/services/phi-redactor.ts`** (A6): two pattern definitions for the same 14 HIPAA identifiers. Drift risk if a new pattern is added on one side only. Long-term cleanup: collapse so server imports from `@shared/phi-patterns`. Both need to be updated in lock-step until then.
...
| **PHI Protection** | `server/services/phi-redactor.ts`, `shared/phi-patterns.ts`, `client/src/lib/sentry.ts`, `server/services/prompt-guard.ts` | 14-pattern PHI redaction for audit logs (server `phi-redactor.ts`); identical pattern set mirrored in `shared/phi-patterns.ts` for client-side use; client-side recursive Sentry scrub via `deepScrubPhi` (depth-bounded, cycle-safe); 16-pattern prompt injection detection + output anomaly scanning (`prompt-guard.ts`) |
...
| **Frontend Hooks** | `client/src/hooks/` | `useWebSocket` (A13 — mount-once-per-mount via refs), `useIdleTimeout` (A16 — fail-closed redirect), `useBeforeUnload`, `useConfig` (A11 — `/api/config` fetch with `FALLBACK_CONFIG`), `useToast` |
...
- `shared/phi-patterns.ts` → consumed by `client/src/lib/sentry.ts` only today; long-term consolidation target for `server/services/phi-redactor.ts` (currently a parallel pattern set — drift risk).
