...
npm run test         # Run backend tests (tsx --test --test-force-exit tests/*.test.ts — 966 tests)
npm run test:coverage # Backend tests with c8 coverage report (text + text-summary)
npm run test:client  # Run frontend tests (Vitest + React Testing Library — 211 tests)
...
### Test Commands
```bash
npm test                   # Backend (966 tests)
npm run test:client        # Frontend (211 tests)
npm run check              # TypeScript type check
```
