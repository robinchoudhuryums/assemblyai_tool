...
- **Backend**: Express.js + TypeScript (ESM), runs on Node. `pdfkit` for server-side PDF report generation (filtered reports, agent profiles, coaching outcomes).
...
npm run backfill-embeddings  # Populate call_analyses.embedding_vec from the JSONB embedding column — run once after pgvector is first enabled (idempotent; batches at 100 rows/pass)
...
| GET | `/api/calls/:id` | authenticated (viewer: own calls only) | Get call details. Viewers get 403 on other employees' calls. When `status === "awaiting_analysis"`, response includes an optional `batchStatus: {state: "pending"\|"submitted", position?, totalPending?, batchIntervalMinutes?, jobId?, submittedAt?} \| null` so the UI can show queue position while the call waits for Bedrock batch processing. |
...
| PATCH | `/api/calls/:id/exclude-from-metrics` | manager+ | Toggle the `excluded_from_metrics` flag. Body: `{ excluded: boolean }`. Flagged calls stay visible in lists/search/detail but are omitted from every aggregate metric (dashboards, leaderboards, filtered reports, badge eval, coaching outcomes, insights). Writes HIPAA audit event `call_excluded_from_metrics` or `call_included_in_metrics`. Requires MFA setup. |
| POST | `/api/calls/bulk-reanalyze` | admin | Re-enqueue historical calls through the AI pipeline. Accepts two payload shapes: `{callIds: [uuid,...]}` (max 50, explicit selection) OR `{filter: {callCategory?, from?, to?, employeeId?, limit? (1-100, default 20)}}` (resolves most-recent completed calls matching filters). Synthetic + excluded_from_metrics calls are already filtered by the underlying `getCallsWithDetails`. Writes audit event `bulk_reanalyze`; resourceId truncated to first 10 IDs for filter-mode runs. Manager-edited analyses preserved (INV-31). 24h rolling per-admin quota (default 200, env `BULK_REANALYZE_DAILY_CAP`); over-quota requests return 429 with `{ quota: { used, cap, retryAfterMs } }` before any AssemblyAI / Bedrock spend is incurred. |
...
| GET | `/api/search/semantic` | authenticated (viewer: own calls only) | Embedding-based semantic search with optional hybrid keyword fusion. Query params: `q` (required, ≤500 chars), `limit` (default 20, max 100), `mode` (`semantic` default, `hybrid`), `alpha` (hybrid mix; 0=keyword 1=semantic, default 0.5), `threshold` (drop results below combined score, default 0.25 — pass 0 to disable), `from`/`to`/`sentiment`/`employeeId` (layered filters applied after scoring). Uses pgvector SQL-native cosine (`<=>` + ivfflat index) when available; falls back to in-memory scoring otherwise. Response: `{ mode, backend, alpha?, threshold, results[], coverage: { totalAccessible, withEmbeddings } }`. Each result carries `similarity`, plus `keywordScore` + `score` in hybrid mode. Query embedding cached per-`q` (200-entry LRU, 5-min TTL). Viewer-scoped via `canViewerAccessCall`. Graceful `mode: "keyword-fallback"` when embedding provider is unavailable. |
| GET | `/api/performance` | manager+ | Performance metrics |
| GET | `/api/reports/summary` | manager+ | Summary report |
| GET | `/api/reports/filtered` | authenticated (MFA-gated) | Filtered reports (query: `from`, `to`, `employeeId`, `role` (preferred) or `department` (deprecated alias), `callPartyType`). Viewers have `employeeId` forced to their linked employee; unlinked viewers get an empty-shaped response. MFA gate matches the CSV/PDF export endpoints so the raw-JSON read path isn't an exfiltration bypass when `REQUIRE_MFA=true`. |
...
| GET | `/api/reports/filtered/export.csv` | manager+ | Server-side CSV export of `/api/reports/filtered` data (summary + sentiment + sub-scores + performers + monthly trends). Writes a first-class `event: "export_report"` HIPAA audit entry. Accepts the same `from` / `to` / `employeeId` / `role` / `callPartyType` query params. |
| GET | `/api/reports/filtered/export.pdf` | manager+ | Server-side PDF export of the same filtered-report data. Same query params as the CSV variant. `event: "export_report"` audit entry with `format=pdf`. |
| GET | `/api/reports/agent-profile/:employeeId/export.csv` | authenticated (viewer: self only) | Server-side CSV export of agent profile (employee meta + call history + topic frequencies). Writes `event: "export_report"` HIPAA audit entry. Requires MFA setup. |
| GET | `/api/reports/agent-profile/:employeeId/export.pdf` | authenticated (viewer: self only) | Server-side PDF export of the same agent profile (tighter summary column than the CSV). `event: "export_report"` audit entry with `format=pdf`. Requires MFA setup. |
...
| GET | `/api/scoring-corrections/similar-uncorrected` | manager+ | Groups the user's recent corrections by (category, direction) with mean-score centroid, requires ≥2 corrections per group, returns up to 5 completed calls per group whose AI performance score is within 0.5 of the centroid AND that this user hasn't edited. Query: `days` (default 30, max 180), `limit` (default 20, max 50). Read-only. Powers the `SimilarUncorrectedCard` widget on `/my-performance`. Tier C #8: grouping + matching logic extracted to pure helpers `groupCorrectionsByCategoryDirection` / `findSimilarUncorrectedCalls` in `server/services/scoring-feedback.ts` for unit-testability. |
| GET | `/api/scoring-quality` | manager+ | Read-only scoring-health dashboard data (Tier A #3). Returns `{correctionStats, qualityAlerts, calibration: {lastSnapshotAt, driftDetected, observedMean, configuredMean}}` — same `checkScoringQuality` + `detectScoringRegression` signals that power `/api/admin/calibration` (admin-only) but with only the lightweight fields managers need. Data is global (not team-scoped); per-team filtering is a deferred follow-on. Powers the `ScoringQualityCard` widget on `/my-performance`. |
...
| GET | `/api/coaching/:id/outcome` | manager+ | Coaching effectiveness: compares N calls before vs after the session. Query: `n` (default 10, range 1-50). Returns avg scores, sub-score deltas, `insufficientData` flag when either window has <3 calls. Tier B #5: filters `excludedFromMetrics` from both windows to match program-wide `/outcomes-summary` behavior — flagged noisy calls no longer distort per-session deltas. |
| GET | `/api/coaching/:id/trajectory` | manager+ | Per-agent post-coaching trajectory chart data. Returns up to N (default 20, max 50) completed scored points on each side of the session's `created_at`, with overall score and per-call sub-scores. Same exclusion semantics as `/outcome` (drops `excludedFromMetrics`). Powers the `TrajectorySection` SVG line chart in the coaching DetailPanel — managers can visually see "did this coaching move the needle?" rather than only seeing the single before/after delta from `/outcome`. |
| GET | `/api/coaching/outcomes-summary` | manager+ | Program-wide coaching effectiveness aggregate over a 90-day window (configurable via `?days`, range 7–365). Optional `?groupBy=manager\|employee` returns per-group breakdown. Optional `?bucket=week` adds a `timeSeries` array of `{bucketStart, measured, avgOverallDelta}` bucketed to UTC-Monday of each week. Optional `?includeSkipped=true` adds a `skipped` array listing sessions omitted for insufficient data with `{sessionId, employeeId, createdAt, reason}` (capped at 100). Every rollup shape carries `avgSubDeltas: { compliance, customerExperience, communication, resolution }` so managers can see which sub-score moved most. Flat shape: `{ totalSessions, measured, insufficientData, positiveCount (delta ≥+0.5), neutralCount, negativeCount (delta ≤-0.5), avgOverallDelta, avgSubDeltas }`. Grouped shape: `{ overall, groups[] }` where each group carries the flat fields + `key` + `label`. Powers the "Program effectiveness" strip + per-manager/agent toggle + sub-score chips + weekly sparkline on the manager coaching page. |
| GET | `/api/coaching/outcomes-summary/export.csv` | manager+ | Server-side CSV export of the per-manager coaching-effectiveness breakdown. Writes a first-class `event: "export_report"` HIPAA audit entry. |
| GET | `/api/coaching/outcomes-summary/export.pdf` | manager+ | Server-side PDF export of the per-manager coaching-effectiveness breakdown — matches the CSV data plus a summary rollup panel. `event: "export_report"` audit entry with `format=pdf`. |
...
| GET | `/api/admin/soft-fail-status` | admin | Soft-fail operator-state dashboard (live observer of the Operator State Checklist). Enumerates 12 silent-degradation conditions across credentials / storage / ai / auth / data / runtime categories, each with status (ok/warning/error/unknown), message, and fix hint. Fast-path only — no blocking network probes. Refreshed on a 30s poll from `/admin/soft-fail`. |
| GET | `/api/admin/pipeline-trends` | admin | Aggregate pipeline observability — today vs. trailing-7-day comparison for completed/failed call counts, avg AI processing duration (`jobs.completed_at − jobs.created_at`), avg cost per call (`usage_records.total_estimated_cost`), plus a 7-day daily cost series. Single SQL roundtrip via `Promise.all` over five indexed-range queries. Falls back to a `backendAvailable: false` zeros/null response when `DATABASE_URL` is unset. Powers the `PipelineTrendsCard` strip on `/admin/health`. |
...
| GET | `/api/admin/cost-budget` | admin | Cost-budget rollup for the spend-tracking page. Returns MTD totals broken out by service (assemblyai / bedrock / bedrockSecondary), trailing-30d total, utilization vs `MONTHLY_COST_BUDGET_USD`, projected month-end at current run-rate, severity (`info` / `warning >75%` / `critical >90%` / `unknown` when budget unset), top-5 spenders, and a count of records with `costPricingMissing=true`. When severity hits `critical`, emits `logger.warn` with `alert: "cost_budget_critical"` for CloudWatch metric filters. Powers the `CostBudgetBanner` at the top of `/admin/spend`. |
...
| GET | `/api/admin/webhooks/stats` | admin | Per-webhook delivery health: `{webhookId, url, active, inFlight, scheduledRetries, deadLettered, delivered24h, avgLatencyMs, circuit: {state, failureCount, lastFailureTime}}`. Queries the `jobs` table grouped by `payload->>'webhookId'` + combines with the in-memory circuit-breaker snapshot. Degrades to zero-counters + `backendAvailable: false` when `DATABASE_URL` is unset. Powers the `/admin/webhooks-health` dashboard. |
| GET | `/api/admin/webhooks/dead-letter` | admin | List of dead-lettered webhook deliveries with body preview (500 chars), attempts, failed_reason, timestamps. Query: `limit` (default 50, max 200). |
| POST | `/api/admin/webhooks/dead-letter/:jobId/retry` | admin | Re-queue a dead-lettered webhook delivery: flips status back to 'pending' with attempts=0. Next job-queue poll picks it up. Does NOT reset the per-webhook circuit breaker. |

**Webhook event types fired by the platform:**
- `call.completed` — payload: `{callId, score, sentiment, duration, employee?, fileName}`
- `call.failed` — payload: `{callId, error, fileName}`
- `score.low` (score ≤4) — payload: `{callId, score, employee?, fileName}`
- `score.exceptional` (score ≥9) — payload: `{callId, score, employee?, fileName}`
- `agent.decline_alert` (Tier 2 #6, from the agent-decline scheduler) — payload: `{employeeId, employeeName, currentAvg, priorAvg, delta, currentCount, priorCount, windowDays, severity: "warning"|"critical"}`

Webhook configs subscribe to events via the `events` array. The `agent.decline_alert` event only fires when `AGENT_DECLINE_CHECK_ENABLED=true` AND at least one webhook subscribes to it — an enabled scheduler with no subscriber produces "fired" log lines but no external delivery.
...
| GET | `/api/admin/simulated-calls/calibration-suite` | Read-only calibration report for presets with `config.expectedScoreRange`. Returns `{presets: [{id, title, qualityTier, expectedMin, expectedMax, actualScore, sentToAnalysisCallId, analyzedAt, status: "pass"\|"fail"\|"not_run", delta}], summary: {total, passed, failed, notRun}}`. Operators use this after prompt template edits or model swaps to detect scoring regressions. Requires a linked analyzed call via prior `/analyze`. |
| POST | `/api/admin/simulated-calls/calibration-suite/run` | Re-analyze every preset with an `expectedScoreRange` against the current prompt template + AI model (Tier A #1, admin, MFA-gated). Deletes each preset's prior analyzed call row (cascades via FK ON DELETE CASCADE to transcript/sentiment/analysis), clears `sentToAnalysisCallId`, then re-enqueues via `sendSimulatedCallToAnalysis`. Audio is reused from S3 (no TTS cost). Returns `202 { presetsTotal, presetsEligible, queued, skipped, skippedReasons }`; analyses complete async over minutes — operators refresh the calibration report to see updated pass/fail. Historical analyses are intentionally not retained — the runner's semantic is "latest prompt vs latest run". Audit event `calibration_suite_run`. |
...
| GET | `/api/gamification/leaderboard/export.csv` | manager+ | Server-side CSV export of the leaderboard (rank, employee, points, avg score, call count, streak, badge count). Writes `event: "export_report"` HIPAA audit entry. Requires MFA setup. |
...
# Agent Decline Alerts (trend-based proactive notification — Tier 2 #6)
AGENT_DECLINE_CHECK_ENABLED        # Set to "true" to enable the scheduler (default: disabled)
AGENT_DECLINE_CHECK_INTERVAL_HOURS # How often to scan active employees (default: 24, range: 1-168). Dedup cool-off is hardcoded at 2× this value — agents stay muted for 2 full cycles (48h default) after an alert before they can re-fire.
AGENT_DECLINE_WINDOW_DAYS          # Length of each half-window compared (default: 14, range: 7-90)
AGENT_DECLINE_THRESHOLD            # Min negative overall delta to fire alert (default: 1.0; ≥1.5 escalates to "critical" severity)
...
# Cost & operations
BULK_REANALYZE_DAILY_CAP        # Per-admin 24h sliding cap on /api/calls/bulk-reanalyze (default: 200, integer >=1). Quota tracked in-process per instance; multi-instance deploys enforce per-instance.
MONTHLY_COST_BUDGET_USD         # Optional monthly budget for the cost-budget banner + critical alert. Unset -> /admin/spend banner shows "monthly budget unset" and `cost_budget_critical` never fires.
UPLOAD_RATE_LIMIT_PER_MIN       # Per-IP rate limit on POST /api/calls/upload (default: 30, was 10 before the batch-upload UX cycle). Manager batch uploads of 10-20 calls are normal usage; 30/min gives margin for normal workflows + the frontend's automatic 429-retry-with-backoff (5s/12s/25s) absorbs cumulative-window edge cases.

# Test infrastructure (dev/CI only — NEVER set in production)
E2E_MOCKS                       # Set to "true" to activate MSW-Node interception of AssemblyAI / Bedrock / S3 / IMDS outbound fetches. Used by Playwright e2e tests via playwright.config.ts. Zero production effect (import gated at `server/index.ts`; `msw` is a devDependency).
...
- **Glass effect + Decorative backgrounds REMOVED** (installment 19) — the prior `[data-glass=*]` overrides and the decorative `<BackgroundLayer>` (hex/softWaves/neonFlow/topoMesh) were obsolete after the warm-paper rewrite and fought the new aesthetic. The glass-override rules were changing `--card` + `--sidebar` via alpha tricks without touching `--background`, which produced a dissonant sidebar-only color change when users toggled glass levels post-redesign. The decorative backgrounds peeked through during page transitions because they sat at `z-0` under the `AnimatedPage` motion.div. Deleted: `[data-glass=*]` CSS rules in `index.css`, the 4 bg-* components (`hex-background.tsx`, `bg-neon-flow.tsx`, `bg-soft-waves.tsx`, `bg-topo-mesh.tsx`), `BackgroundLayer` in `App.tsx`, `background` + `glass` fields from `AppearancePrefs`, the Background + Glass panels in settings.tsx, `backdrop-blur-xl` classes from `sidebar.tsx` + `components/ui/card.tsx`, and `border border-white/30 dark:border-white/[0.08]` from the Card primitive (replaced with `border-border`). Storage migration: saved prefs with `{ background, glass }` keys load cleanly — the extra fields are silently dropped during load validation.
...
- **`updateCall` is employeeId-free AND unknown-key-throwing** (A6/F14, extended by F-04): all three storage backends throw if `employeeId` appears in the updates payload. The manager-facing PATCH /api/calls/:id/assign route uses the new `setCallEmployee` method; pipeline auto-assignment uses `atomicAssignEmployee`. Closes a silent-clobber race where status updates would re-write a stale `employee_id` from a prior read. **F-04**: `updateCall` now ALSO throws on any key not in `UPDATABLE_CALL_KEYS` — previously PostgresStorage silently dropped unknown keys (e.g. `externalId`) while MemStorage/CloudStorage silently wrote them. The single source of truth is `UPDATABLE_CALL_KEYS` in `server/storage.ts`. Adding a new updatable Call column requires adding it to BOTH that allowlist AND PostgresStorage's `COLUMN_MAP`.
...
- PostMessage vocabulary: inbound `embed:ready` (RAG → CA, flips the drawer's loading state off), `embed:close` (RAG → CA, fired on Escape inside the iframe which doesn't bubble — drawer closes via `onClose`), `embed:open-source {url, preview?: {title?, snippet?}}` (RAG → CA, fired when a source citation anchor is clicked — url is origin-validated against the iframe's origin to prevent redirect-laundering. When the optional `preview` field is included, the drawer renders an inline preview panel anchored to the iframe bottom with the snippet + an "Open in new tab" escape button + a close button, so the user can read the supporting snippet without leaving CA's context. Without `preview`, the legacy behavior fires: CA window.open's the url in a new tab so PDFs don't render in the 420px drawer. RAG can opt into the inline preview by adding the field; absence stays backward-compatible); outbound `embed:clear` (CA → RAG, triggers a ChatInterface remount via the `ArrowCounterClockwise` button). Origin validation on both sides: inbound messages are gated on `event.origin === iframe's origin`; outbound `postMessage` targets that same origin (not `*`).
- RAG must set `EMBED_ALLOWED_ORIGIN=https://umscallanalyzer.com` for the iframe to load at all (CSP `frame-ancestors` gate).
- CA's HTML CSP includes a `frame-src` directive derived from `RAG_SERVICE_URL` at request time (`server/vite.ts:injectCspNonce`). Unset or malformed → falls back to `'none'` (deny by default). Both sides must permit the framing: RAG's `frame-ancestors` authorizes CA as parent, CA's `frame-src` authorizes RAG as child.

## Common Gotchas
- **Pipeline + service fire-and-forget catches now log** — every `.catch(() => {})` silent-no-op in pipeline / coaching webhook trigger / startup hydration loaders / storage cleanup paths has been replaced with `logger.warn` (or `logger.debug` for best-effort cleanups where 404 is expected, e.g. retention purge of a never-existed batch-pending file). External CloudWatch metric filters can now match `startup: failed to hydrate ...` / `coaching: webhook delivery failed` / `retention: batch-pending artifact cleanup non-fatal` to surface previously-invisible degradation. The `LOG_LEVEL=debug` envelope is required to see the storage-cleanup variants; default level still suppresses them so quiet ops stay quiet.
- **`ScoreReasoningPanel` ("Why this score?") is presentation-only** (`client/src/components/transcripts/side-rail.tsx`) — the collapsible expander on the transcript side rail derives its narrative from existing `subScores` + `flags` + `feedback` fields already loaded with the call. There is no `reasoning` field in the AI prompt schema or in storage; future maintainers wondering "where does this text come from?" should look at `deriveScoreDrivers()` in side-rail.tsx, not the Bedrock prompt. Adding actual chain-of-thought from the model would require a prompt template change, a new storage field, and re-running historical analyses to populate it.
- **Webhook circuit breaker 4xx-not-counting semantics** (F-02) — `deliverWebhook` throws a typed `WebhookHttpError` (carrying numeric `status`); `deliverWithRetry` always passes `{ isFailure: isWebhookCircuitFailure, ... }` to `webhookBreaker.execute()`. 4xx responses (except 429) do NOT count toward the 5-failure open threshold, so a misconfigured webhook URL returning 404 fails each delivery but doesn't also block the durable-retry enqueue for the 5-minute cooldown. Network errors, timeouts, 429, and 5xx still count. Mirrors Bedrock's INV-32 treatment; not yet codified as a separate invariant.
- **`firstFeedbackText()` handles both feedback shapes** (`client/src/pages/my-performance.tsx`) — AI feedback items arrive as EITHER plain strings OR `{ text, timestamp? }` objects depending on whether the model emitted a timestamp. The earlier string-only filter silently missed object-shaped items, causing the exemplar's "One thing to try" (and "What worked well") slots to frequently render blank. Any new consumer extracting a representative example from `feedback.strengths` / `feedback.suggestions` should use this helper (or the analogous `inlineFeedbackBySegment` pattern in `transcript-viewer.tsx`) rather than `.find(x => typeof x === "string")`.
- **Transcript viewer inline AI feedback** (`client/src/components/transcripts/transcript-viewer.tsx`) — AI feedback with parseable timestamps now renders as sage/amber-stripe callouts directly under the matching transcript segment AND in the side-rail `CoachingPanel`. The `inlineFeedbackBySegment` useMemo indexes items by segment via the shared module-level `parseAiTimestampToMs` helper. Feedback items without a timestamp stay in the side rail only. The callout's `onClick` does `e.stopPropagation()` so it doesn't bubble into the parent segment's seek handler.
- **Scrubber is `position: sticky bottom-0`, not inline-at-end** — `client/src/components/transcripts/transcript-viewer.tsx:1480-1532` wraps the bottom-docked `<Scrubber>` in `sticky bottom-0 left-0 right-0 z-20 bg-card`. The "bottom-docked" naming has been in CLAUDE.md installment 4 documentation since `28f844c`, but until the recent fix the dock was just `mt-5 pt-4 border-t` — an inline block at the very end of the ~1500-line page flow that ended up below the fold on long calls and looked completely missing. The wrapper now also renders a visible "Audio could not be loaded" fallback when `audioReady` is false, so genuine load failures (404, decode error) stop being silent. The dock always renders; the inner content swaps based on `audioReady`.
- **`/admin/soft-fail` is the live view of the Operator State Checklist** (`client/src/pages/soft-fail-status.tsx` + `GET /api/admin/soft-fail-status`) — admin-only page with 30s auto-refresh that enumerates 12 silent-degradation conditions across credentials / storage / ai / auth / data / runtime categories. Each check reports status (ok / warning / error / unknown), a plain-English message, and a fix hint. Fast-path only: no blocking network probes, so it's cheap to refresh. The Operator State Checklist section below remains the authoritative spec; the page is the observer.
...
- **Webhook delivery is durably retried via the job queue** (`server/services/webhooks.ts:deliverWithRetry` + `deliver_webhook` job type in `server/routes.ts`) — after the 4 in-process retries (2s/4s/8s/16s backoff) exhaust, `deliverWithRetry` calls the injected `enqueueWebhookRetry` handler which enqueues a `deliver_webhook` job. The job queue adds up to 3 more attempts with its own retry + dead-letter semantics, so a webhook outage survives a deploy. Without `DATABASE_URL` (dev), `setWebhookRetryEnqueuer` is never wired and the exhaustion path stays a `logger.error` with no durable retry (prior behavior preserved). On redelivery, `redeliverWebhook` re-reads the live webhook config — a URL/secret change or a flip of `active=false` between the original attempt and the durable retry takes effect, and a config whose `events` array no longer subscribes to the event will dead-letter the job.
- **`jobQueue.stop()` drain budget is 20s** (raised from 15s in `server/index.ts:621-632`) — nested inside the 30s outer hard-exit. Scheduler stops preceding this are synchronous `clearInterval` calls so they consume ~no budget; the 20s gives in-flight audio pipeline jobs more time to complete before the DB pool closes under them.
- **`validateTimestamps` flag emission is single-flag-idempotent** (`server/services/ai-provider.ts:309-320`) — on re-validation, strips any pre-existing `output_anomaly:invalid_feedback_timestamps:*` and emits one fresh flag with the current `strippedCount`. Replaces the prior prefix-skip dedup that could retain a stale count if the analysis is re-validated.
- **`embeddingCosineSimilarity` is a shared export** (`server/services/call-clustering.ts:206`) — used by the in-memory fallback path of `/api/search/semantic`. The pgvector fast path uses SQL-native `<=>` (cosine distance operator) instead and never touches this helper. Pure function, no side effects.
- **Semantic search supports three modes + a relevance threshold** — `/api/search/semantic` accepts `?mode=semantic\|hybrid` (default semantic) and `?alpha=0-1` (hybrid mix, default 0.5 semantic / 0.5 keyword). `?threshold=N` (default 0.25) drops results below the combined score on BOTH modes — pass `0` to disable pruning. Response includes `backend: "pgvector" \| "in-memory"` so clients can detect which path was taken. API callers that previously expected unlimited top-N must set `?threshold=0` explicitly; the bundled UI does not.
- **Query embedding cache lives in `routes/reports.ts`** — 200-entry LRU with 5-min TTL, keyed on the raw query string. Distinct from the call-embedding cache in `bedrock.ts`. Case-sensitive — `Billing` and `billing` cache separately; intentional to preserve proper-noun exactness.
- **pgvector migration runs at startup** (`server/db/pool.ts:runMigrations`) — tries `CREATE EXTENSION IF NOT EXISTS vector` + `ALTER TABLE call_analyses ADD COLUMN embedding_vec vector(256)` + an ivfflat cosine index. Sets the `pgvectorAvailable` module flag on success. If the extension isn't available on the DB (e.g. RDS PostgreSQL <15.2), `isPgvectorAvailable()` returns false, `updateCallAnalysis` silently skips the vector dual-write, and `/api/search/semantic` falls through to in-memory scoring. Dimension is 256 to match the Bedrock Titan Embed v2 configuration in `bedrock.ts:340` (`dimensions: 256, normalize: true`).
- **`updateCallAnalysis` dual-writes `embedding_vec` when pgvector is available** (`server/storage-postgres.ts`) — writes BOTH the JSONB `embedding` column (for the in-memory fallback / back-compat) and the pgvector `embedding_vec` column. Historical rows written before pgvector landed won't have `embedding_vec` populated until `npm run backfill-embeddings` runs once. New calls are dual-written automatically.
- **`JobQueue.enqueue(type, payload, options)` accepts `{delayMs}`** (`server/services/job-queue.ts`) — defers first pickup by N ms via `locked_at = NOW() + interval`. Back-compat: old callers passing a number as the third arg get treated as `priority` (same as before). Used by the webhook persistent-retry handoff to delay the first job-queue attempt by 60s so a down receiver isn't hammered at the poll interval.
- **Shared report-export helpers in `routes/utils.ts`** — CSV: `buildCsv(sections)` + `writeCsvResponse(res, csv, filename, audit?)`. PDF: `buildPdfBuffer(sections, metadata)` + `writePdfResponse(res, buf, filename, audit?)`. Both accept the SAME `CsvSection[]` shape — a single aggregation can feed both formats. `CsvSection.chart` is an optional PDF-only field (`PdfChartSpec`: `line` or `bar`) rendered above the table via `renderChart()` using pdfkit path primitives; ignored by `buildCsv`. Line charts draw a dashed zero baseline when zero is in range; bar charts auto-paginate on overflow. Null/NaN points are skipped (line) or render as "—" (bar). `buildPdfBuffer` lazy-imports `pdfkit` so test runs and the audio-pipeline boot path don't pay the font-load cost. Add new export endpoints via these helpers rather than hand-rolled string concatenation or direct pdfkit usage.
- **Per-webhook circuit breaker** (`server/services/resilience.ts:PerKeyCircuitBreaker` + `webhookBreaker` in `webhooks.ts`) — opens after 5 consecutive failures per webhookId for 5 minutes, then transitions to half-open for one test call. An OPEN circuit skips BOTH delivery AND the durable-retry enqueue so a known-broken receiver doesn't accumulate wasted queue entries. State is in-memory only — resets on process restart (acceptable because the 5-min cooldown is short). Keyed map is LRU-bounded at 1,000 entries. Surfaced via `GET /api/admin/webhooks/stats` + the `/admin/webhooks-health` admin dashboard.
- **`PerKeyCircuitBreaker` is the keyed variant of `CircuitBreaker`** — use it when the same failure semantics need to apply per-target (per webhook URL, per RAG endpoint, etc). The single-key `CircuitBreaker` + `bedrockCircuitBreaker` singleton pattern is for shared-target systems (Bedrock is one service regardless of caller). Don't conflate the two — sharing one `CircuitBreaker` across multiple targets would let one bad target brownout all the others.
- **`PerKeyCircuitBreaker.execute(key, fn, options)` accepts per-key threshold/resetMs overrides** — options can be either a plain `isFailure` predicate (back-compat) OR an object `{ isFailure?, threshold?, resetMs? }`. Overrides are applied ONLY on first breaker creation per key; a later policy change requires `reset(key)` to recreate the breaker with new thresholds. The webhook retry-policy path uses this to let mission-critical consumers set tighter circuits.
- **Per-webhook `retryPolicy` overrides** (`shared/schema.ts:webhookRetryPolicySchema`) — optional `{ maxInProcessRetries?, circuitThreshold?, circuitResetMs? }` on `WebhookConfig`. Unset fields inherit service-wide defaults (4 in-process retries, circuit opens at 5 failures for 5 min). Zod schema clamps to sane bounds (retries 0-10, threshold 1-50, reset 10s–1h) so operator error can't create DoS vectors. `PATCH /api/admin/webhooks/:id` calls `webhookBreaker.reset(id)` when `retryPolicy` changes so new thresholds take effect immediately instead of waiting for LRU eviction.
- **`getCallsWithDetailsByIds(ids)` is the bulk-hydration primitive** (`server/storage.ts`) — used by `/api/search/semantic`'s pgvector fast path to hydrate only the candidate IDs (~60 rows) instead of scanning all completed calls. PostgresStorage runs a single `id IN ($1,...)` + the standard joins; MemStorage + CloudStorage filter the in-memory/scan output. Synthetic calls excluded in all three backends (INV-34). Empty input returns an empty array without hitting the DB.
- **`getCoachingOutcomes(windowStart, perSessionN?)` is SQL-native on Postgres** — uses a CTE + `ROW_NUMBER() OVER (PARTITION BY session_id, bucket)` window function to rank top-N completed calls on each side of each session in one query. Replaces the prior N+1 pattern (one `getCallsWithDetails` per unique employee in the windowed sessions) — eliminates ~25k call rows loaded into memory at 500-session scale. MemStorage + CloudStorage fall back to the shared `computeCoachingOutcomesInMemory` helper (module-level in `server/storage.ts`). The `/api/coaching/outcomes-summary` JSON route + both exports call through this method; the CSV + PDF exports share a `computeManagerOutcomeRows(windowDays)` helper in `routes/coaching.ts`. Only sessions with at least one matching call appear in the result — callers combine with `getAllCoachingSessions()` to identify zero-call "insufficient" sessions.
- **`subsystems.onboarding.chronicallyUnlinkedLast7d` on `/api/admin/health-deep`** — distinct-username count from `audit_log` where `event = 'user_employee_link_unresolved'` in the last 7 days. Null when `DATABASE_URL` is unset or the query fails. Threshold 3+ raises an entry in the `issues` list so small deployments don't get spammed. Surfaced as the Onboarding SubsystemCard on `/admin/health`.
- **MSW-Node e2e mocks** (`server/test-mocks/{handlers,setup}.ts`) — activated when `E2E_MOCKS=true` via a conditional dynamic import at the top of `server/index.ts`. Intercepts outbound fetch to AssemblyAI (upload + submit + poll), Bedrock (Converse + Titan embed), S3 PUT/GET, and IMDS so Playwright e2e tests can drive the full audio pipeline without live credentials. `onUnhandledRequest: "bypass"` lets unmocked fetches (WebSocket, localhost) pass through. `TEST_FIXTURES` export exposes the deterministic transcript ID / URL / text / analysis for test assertions. Zero production path — the import is gated behind the env flag and `msw` is a devDependency. `playwright.config.ts` sets the flag + dummy AWS / AssemblyAI creds so provider init doesn't refuse.
- **`/api/reports/export-beacon` is DELETED** (previously shipped as the client-side-export audit trail) — replaced by server-side export endpoints that emit `event: "export_report"` synchronously. Client-built TXT + CSV downloads on the reports page are also gone; the "Report" button was retired and `handleDownloadCSV` now routes to `/api/reports/filtered/export.csv` or `/api/reports/agent-profile/:id/export.csv`. Old audit_log rows with `event = 'export_report_clientside'` remain in historical data — operators querying historical exports should match on both event strings.
- **Coaching outcomes endpoint is multi-dimensional** (`server/routes/coaching.ts:outcomes-summary`) — `?groupBy=manager\|employee` for per-group breakdowns, `?bucket=week` for UTC-Monday weekly time-series, `?includeSkipped=true` for insufficient-data drill-in (capped at 100 sessions). Every rollup shape carries `avgSubDeltas` (compliance/customerExperience/communication/resolution). Adding a new pivot requires extending the `PerSession` / `rollup()` types — the response shape is additive across all variants so existing clients are unaffected.
- **`POST /api/users/:id/link-employee` updates displayName only** (`server/routes/users.ts`) — links a user to an employee by setting `users.display_name = employees.name`, which makes `getUserEmployeeId()` resolve via the name-match branch on the next request. Intentionally writes the USER side (not `employees.email`) because the user record is auth-only and changing displayName is safer than changing employee email (which may be consumed by other downstream joins). Audit event `user_employee_link_created`.
- **`AIAnalysisProvider` interface has optional `generateEmbedding?`** (`server/services/ai-provider.ts:37`) — Bedrock provider implements it; other providers may opt out. Callers must guard via `aiProvider.generateEmbedding ? await aiProvider.generateEmbedding(text) : null`.
- **`transcript-viewer.tsx` volume persist uses `safeSet`** — direct `localStorage.setItem` was replaced with `safeSet("transcript-audio-volume", ...)` so Safari private mode + quota-full scenarios fail quietly via the shared wrapper (consistent with the rest of the client-side preference persistence).
...
- **Simulated Call Generator warm-paper rewrite landed** (installment 18) — `client/src/pages/simulated-calls.tsx` now matches the document aesthetic used across the rest of the app. Added app-bar breadcrumb (Dashboard › Admin › Simulated Calls), a proper page header with accent-tinted `Microphone` glyph, replaced the shadcn `Tabs` with the `ToolbarTab` mono-uppercase button row (Library with count + Generate New), and swapped all four `Card` blocks (Script, Audio Quality, Circumstances, Summary) for a page-local `Panel` primitive with `kicker` / `title` / `description` / `action` / `dense` props. New helpers: `SectionKicker`, `Panel`, `ToolbarTab`, `SpeakerTag` (tonal per speaker — agent=copper, customer=sage, hold=muted, interrupt=amber), `CircumstanceKindPill` (Rule=copper, AI=sage). Killed all prior hardcoded Tailwind colors: `text-green-600` → `var(--sage)` via inline style; `border-purple-*` / `text-purple-*` banner chrome → accent-tinted warm-paper panel with `boxShadow: inset 2px 0 0 var(--accent)`; `text-amber-600` → `var(--amber)`. Kept Dialog/Popover/Input/Textarea/Slider/Select shadcn primitives (they're field-level controls, not chrome). Net: 0 remaining shadcn `Card*` / `Tabs*` / `Badge` in `simulated-calls.tsx`. Two `rounded-md` sub-sections (per-turn voice settings + voice picker rows) were left as-is because they use token-based backgrounds (`bg-muted/50`) — acceptable, and promoting them to inline panels would bloat the diff. All 202/202 frontend tests still pass.
- **Accent palette is user-switchable via Settings → Accent** (installment 17) — `client/src/lib/palettes.ts` is the single source of truth. The entire accent surface (nav, score tiles, chart-1, sidebar stripes, focus rings) flows from two CSS tokens: `--copper` + `--copper-soft` (each with a `:root` and a `.dark` value). When the user picks a non-default palette, `AppearanceProvider` injects a `<style id="palette-override">` block redefining those four values; downstream aliases (`--primary` / `--accent` / `--ring` / `--chart-1` / `--sidebar-*`) resolve automatically. Adding a new palette is a one-file change: append to `PALETTES` in `palettes.ts`. Two caveats: (1) the variables are still called `--copper`/`--copper-soft` even when resolved to blue/green — historical artifact; a rename to `--accent-source` is a deferred follow-up. (2) `--chart-4` is a hardcoded OKLCH blue in `index.css`; on the blue palettes it lands near the accent, so on 4+ series charts it's a mild collision. `--chart-1` (accent) is the primary line/bar and is unaffected. Ships with 6 options: `copper` (default) / `medicalBlue` / `corporateBlue` / `skyBlue` / `indigo` / `sage`.
- **Sidebar app-rail uses the same warm-paper primitives as the redesigned pages** (installment 16) — `client/src/components/layout/sidebar.tsx` now follows the document aesthetic: mono kicker + display-font brand title with a copper accent dot; nav items swap solid-copper active fill for `bg-[var(--copper-soft)] text-foreground font-medium shadow-[inset_2px_0_0_var(--accent)]`; section labels (Analytics / Management / Admin) use `font-mono uppercase` at `fontSize: 10` `letterSpacing: 0.08em`; quick-switch and user card footer get hairline-bordered panels with copper-soft initials discs. The active-state pattern (`copper-soft` bg + inset left stripe + foreground text) is the convention any other custom nav widgets should follow.
...
- **Transcript Viewer event bus** — `ViewerHeader` (app bar) and `SideRail` still coordinate with `TranscriptViewer` (body) via three `window` CustomEvents: `transcript:export` (header button → viewer's `handleExportTranscript` via ref — ref indirection avoids stale-closure capture of `call`/`transcriptSegments` in the empty-deps listener effect), `transcript:download` (header button → viewer's `handleDownloadAudio`, same ref pattern), and `transcript:role-change` {role: "agent"|"manager"} (header toggle → side rail's coaching panel label swap). The ref bridge is load-bearing — moving the listener `useEffect` to depend on `call` or `transcriptSegments` would re-add the listener on every render and multiply event handling. **Search is no longer part of the bus**: the input was moved from `ViewerHeader` into the transcript body (directly above the transcript container) and now drives local `searchQuery` state directly; the `transcript:search-query` listener was retained for backward-compat but is no longer the primary driver. New controls added to the header should follow the ref-via-event pattern, not introduce prop-drilling through a page-level container.
- **Ctrl/Cmd+F in the Transcript Viewer focuses `#transcript-header-search`** — the input with that id now lives in the transcript body (moved out of `ViewerHeader` in the 2026 relocation pass). Keyboard shortcut remains `document.getElementById("transcript-header-search").focus()` + `.select()`. The id itself is the contract — any refactor of the transcript body's search bar must preserve it. (The id name retains "header" for historical reasons; a rename would ripple through keyboard-shortcut code and is deferred.)
...
- **`failJob` is guarded on `status='running'`** (F3) — the attempts-increment UPDATE has `AND status = 'running'` so a race between `reapStaleJobs` SELECT and a concurrent `completeJob` UPDATE no longer flips a just-completed job to 'pending' or 'dead'. Reaper invocations on already-completed jobs silently no-op. Covers the narrow window where a stale-heartbeat reap races the worker's successful completion.
- **X-Forwarded-For IPv4 octets are strictly bounded** (F1) — the regex at `server/index.ts` validates each octet as `(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d)` instead of the prior `\d{1,3}`. Out-of-range octets (999.x, 256.x) and leading-zero notation (01.02.03.04) are now stripped from the header, keeping downstream rate-limit keys + audit IP fields accurate. Canonical IPs are unaffected.
- **`getAwsCredentials()` falls back to cached creds on IMDS refresh failure** (F4) — when `now >= cachedExpiration - REFRESH_BUFFER_MS` and the IMDS call throws, the function returns `cachedCredentials` as long as they haven't truly expired (`now < cachedExpiration`). Previously returned `null` in the pre-expiration window even though valid creds were in hand — caused brief AWS outages during transient IMDS flaps. `imdsAvailable` stays `true` during the fallback window so the next refresh failure continues to log at `warn` rather than degrading to `debug`.
- **Batch result processor closes missing-callIds loop** (F2) — after iterating the `results` map in `processBatchResults`, any callId present in `job.callIds` but absent from `results` (partial Bedrock output, single-record parse failure) is now marked `failed` + broadcast + pending-item deleted, rather than stranding in `awaiting_analysis` for the 2-hour orphan-recovery window. Skips calls already in terminal state (completed/failed).
- **Best-practice ingest is flag-gated** (F11) — `pipeline.ts:720-746` skips `ingestBestPractice` when `analysis.flags` contains `prompt_injection_detected`, any `prompt_injection*` prefix, or any `output_anomaly*` prefix. Defense-in-depth on top of the existing `<<<UNTRUSTED_KNOWLEDGE_BASE>>>` wrapper at query time — a high-score call whose score was inflated by a successful injection payload no longer lands in the KB.
- **`calls.excluded_from_metrics` mirrors the synthetic-filter pattern** — manager-flagged calls stay visible in list/search/detail paths but are omitted from every aggregate read path (dashboards, leaderboards, filtered reports, badge eval, coaching outcomes, insights, sentiment distribution, top performers). Applied in `storage.ts` + `storage-postgres.ts` alongside `synthetic = FALSE` at every aggregate query site and in `server/routes/analytics.ts`'s 11 inline SQL queries. Toggled via `PATCH /api/calls/:id/exclude-from-metrics` (manager+, MFA-gated). Covered by 22 assertions in `tests/synthetic-call-isolation.test.ts`.
- **`coaching_sessions.effectiveness_rating` + `effectiveness_note` are nullable** (Tier 2 #8) — captured via the existing PATCH `/api/coaching/:id` endpoint (rating enum: `helpful` | `neutral` | `not_helpful`; note ≤1000 chars). Pre-existing sessions have NULL values. Surfaced in the Coaching detail panel as a new "Effectiveness rating" section shown only on completed sessions.
- **`getBatchQueueStatus(callId)` is a read-only helper** — exported from `server/services/batch-scheduler.ts` and consumed by `GET /api/calls/:id` when the call is in `awaiting_analysis`. Scans `batch-inference/active-jobs/` + `batch-inference/pending/` in S3 and returns `null` on any error; never mutates state. Response shape: `{state: "pending"|"submitted", position?, totalPending?, batchIntervalMinutes?, jobId?, submittedAt?}`.
- **Agent decline scheduler is off by default** (Tier 2 #6) — `AGENT_DECLINE_CHECK_ENABLED=true` enables the scheduler. When disabled, `startAgentDeclineScheduler()` logs "scheduler disabled" and returns a no-op stopper. The `agent.decline_alert` webhook event only delivers if at least one webhook config subscribes to it — enabling the scheduler without a subscriber produces internal log lines but no external notification.
- **Agent-decline dedup is DB-backed with a 2× interval cool-off** (Tier A #2) — `agent_decline_alert_history` table persists `(employee_id, last_alerted_at)` when `DATABASE_URL` is set. Dedup cool-off is hardcoded at `2 × AGENT_DECLINE_CHECK_INTERVAL_HOURS` (default 48h), so an alerted agent stays muted for 2 full cycles before re-alerting. A process restart no longer re-fires alerts for currently-declining agents. Falls back to the prior in-memory `previouslyAlerted` set when DB is unavailable. Log line includes `dedupBackend: "db" | "in-memory"`. Opportunistic prune of rows older than the cool-off runs at each cycle start; table stays bounded.
- **`CircuitBreakerOpenError` is the typed open-circuit rejection marker** (Tier C #10/F5) — `CircuitBreaker.execute()` throws this class (instead of a generic Error) when rejecting a call due to open state. Callers can `err instanceof CircuitBreakerOpenError` to cleanly detect "rejected by policy" vs. "upstream errored". `webhooks.ts` retry loop uses this to abandon in-process retries when the breaker trips mid-sequence — previously relied on a race-prone second `isOpen()` query after a failed execute. String-matching on the old "Circuit breaker ... is open" message would now fail — grep shows no existing callers do that.
- **Calibration suite runner deletes prior analyzed calls** (Tier A #1) — `POST /api/admin/simulated-calls/calibration-suite/run` deletes the `calls` row previously linked via `simulated_calls.sent_to_analysis_call_id` before re-enqueuing. Cascades via FK ON DELETE CASCADE to transcript/sentiment/analysis. Historical analyses per preset are NOT retained; the runner semantic is "latest prompt vs latest run". If a future migration drops the FK cascade, the runner will leak orphan transcript/sentiment/analysis rows.
- **Calibration-drift alert tag is a CloudWatch-queryable signal** (Tier C #9) — `server/services/calibration-assertions.ts` emits `logger.warn("calibration drift detected", { alert: "calibration_drift", callId, presetId, expectedMin, expectedMax, actualScore, delta, ... })` when a synthetic call's performance score falls outside its preset's `expectedScoreRange`. CloudWatch metric filters matching `{ $.alert = "calibration_drift" }` wire this into the existing alerting stack. Fires only for synthetic calls; non-synthetic paths skip the assertion entirely. Fire-and-forget — assertion failures (DB unavailable, preset missing) log at `debug` and don't affect the pipeline.
- **`recurringWeaknessInFlight` mutex** (Tier C #11/F6) — `coaching-alerts.ts` keeps a module-level `Map<employeeId, timestamp>` with 5-min TTL. Top of `checkRecurringWeaknesses` checks + sets the mutex; finally-block clears on success OR failure. Two concurrent pipeline completions for the same employee no longer both pass the 7-day spam guard and create duplicate plans — the second invocation returns early. TTL is the safety net for a crashed process between check and insert. "Legitimate second detection" within 5 minutes is deferred to the next trigger; acceptable per the existing spam-guard intent.
- **ElevenLabs retries 3× with jitter on 429** (Tier D #16/F8) — `elevenlabs-client.ts` used to do one fixed-2s retry on rate-limit. Now retries at `[1000ms, 2000ms, 4000ms]` with ±20% jitter. De-correlates concurrent workers hitting the limit simultaneously so they don't all retry at T+2s and collide again. Worst-case total delay ~7s; admin-only daily-capped feature so no user-facing impact.
- **`/api/coaching/:id/outcome` filters `excludedFromMetrics`** (Tier B #5) — per-session outcome now matches program-wide `/outcomes-summary` exclusion behavior. `getCallsWithDetails` intentionally does NOT filter excluded calls (they stay visible in list/search), so the per-session route adds an inline `.filter(c => !c.excludedFromMetrics)` to the before/after windows. Flagged noisy calls no longer distort the delta math here.
- **`resolveBulkReanalyzeCallIds` + scoring-feedback helpers are extracted pure functions** (Tier C #8) — `server/routes/utils.ts` exports `resolveBulkReanalyzeCallIds(candidates, filter)`; `server/services/scoring-feedback.ts` exports `groupCorrectionsByCategoryDirection(corrections, minCount)` and `findSimilarUncorrectedCalls({groups, calls, username, alreadyCorrectedCallIds, windowScore?, perGroupLimit?, totalCap?})`. Route handlers (`/api/calls/bulk-reanalyze`, `/api/scoring-corrections/similar-uncorrected`) now compose them. 29 unit tests in `tests/scoring-feedback-helpers.test.ts` + `tests/bulk-reanalyze-filter.test.ts` cover the semantics (grouping math, category generalization, score proximity, edit-skip, per-group + total caps, filter composition, limit clamping).
- **Synthetic calls get a post-analysis calibration check** (Tier C #9 pipeline hook) — after a synthetic call transitions to "completed" status, `pipeline.ts` dynamically imports + calls `checkCalibrationAssertion({callId, performanceScore})`. Non-synthetic calls skip. The dynamic import keeps the calibration module out of the hot path's startup graph. Errors in the assertion are non-blocking (`debug` log).
- **Mobile support tier is Tier 1 — "usable on mobile"** — sidebar is a mobile drawer (hamburger in `App.tsx` drives `sidebarOpen` state; `lg:hidden` fixed-position button at `top-3 left-3` with 44×44 tap target; `lg:relative` persistent rail on ≥lg viewports). Agent-facing pages (`upload`, `transcripts`, `agent-scorecard`, `my-performance`) use `px-4 sm:px-7` responsive padding to recover content width on ~375px viewports. Coaching detail panel is full-width via `width: min(720px, 100vw)` with a 44×44 close button because the overlay backdrop is invisible on mobile (the panel covers the entire viewport). Tier 2 deferred: hamburger/app-bar overlap resolution, transcript viewer mobile reordering (score-first), calls-table stacked-row-card layout, admin-page responsive sweep.
- **44×44 touch-target minimum on custom buttons** (mobile Tier 1) — WCAG 2.5.5 / Apple HIG. Applied to the hamburger button (`App.tsx`) and the coaching detail panel close button (`detail-panel.tsx`). Any NEW icon-only button in a mobile-reachable flow should meet this minimum via `min-w-[44px] min-h-[44px] inline-flex items-center justify-center`. Shadcn primitives (`<Button>`) already meet this via default size; the concern is hand-rolled `<button className="p-2">` patterns which end up around 32-36px.
- **Responsive padding convention** — agent-facing pages use `px-4 sm:px-7` (16px on mobile, 28px on desktop). Admin pages still use bare `px-7` (`admin.tsx`, `agent-compare.tsx`, `batch-status.tsx`, etc.) — desktop-tuned intentionally for Tier 1 scope. When adding a new page, follow `px-4 sm:px-7` for agent-facing flows (viewer-visible or manager daily-driver) and keep `px-7` for admin-only surfaces.
- **Hamburger button visually overlaps page app-bars on mobile** — known Tier 2 trade-off. The fixed-position hamburger at `top-3 left-3` occupies `~x=12..56, y=12..56`; page app-bars (`px-4 sm:px-7`) start content at `x=16` on mobile, so breadcrumbs can collide visually behind the hamburger's card-background pill. Legible but ugly. Fix requires either (a) relocating hamburger to top-right, (b) adding `pl-16` to every mobile app-bar, or (c) extracting a shared AppBar component that accounts for the hamburger. All three are Tier 2 scope.
- **No `useMediaQuery` / `use-mobile` hook exists** (A31 deleted the shadcn variant) — all responsive behavior goes through Tailwind media-query classes (`md:`, `lg:`, `sm:`). If a component genuinely needs a JS breakpoint check (for behavior, not layout), add a fresh hook rather than resurrecting the deleted shadcn one — the deleted file was 70+ lines for a single 768px boolean, which is cheaper as `window.matchMedia` inline.
...
- **estimateBedrockCost returns `number | null`** (A27) — unknown models return null. Unknown-model usage records store `estimatedCost: null` + `costPricingMissing: true` (F6 — was `0` before). The spend dashboard's `formatCostPrecise` renders `—` for null and the spend table badges affected rows with `pricing?`; `totalEstimatedCost` still rolls up to a number (null contributes 0), so the dashboard total bars underreport the model side until pricing is added — the per-row badge is the visibility surface. Adding a new BEDROCK_MODEL requires updating `BEDROCK_PRICING` in `server/routes/utils.ts`. No longer silent: `server/index.ts` startup warns at boot if `BEDROCK_MODEL` is set but absent from the pricing table, and `warnOnUnknownBedrockModel()` in `routes/utils.ts` fires a once-per-model `logger.warn` from the pipeline cost-tracking path.
- **F1: CloudStorage aggregate paths now filter `excluded_from_metrics`** — `getDashboardMetrics`, `getSentimentDistribution`, `getTopPerformers`, `getInsightsData`, `getLeaderboardData`, `getFilteredReportMetrics`, `countCompletedCallsByEmployee`, and `getRecentCallsForBadgeEval` in CloudStorage now apply the manager-excluded filter on top of the synthetic filter that `getAllCalls` inherits. Closes the INV-34/35 gap that previously affected only the `s3-legacy` opt-in backend (PostgresStorage and MemStorage already filtered correctly). `getCallsByStatus`/`findCallByContentHash`/`findCallByExternalId` bypass the centralized `getAllCalls` filter to preserve orphan recovery + dedup against synthetic rows.
- **F3: `validateUrlForSSRF` warn-and-allows on DNS lookup failure** — `server/services/url-validator.ts:138-155`. Prior behavior rejected URLs whose DNS query threw, which blackholed webhook config creation during transient resolver outages and produced a fan-out of "URL hostname could not be resolved" errors with no actionable signal. Now we `logger.warn("url-validator: DNS lookup failed, allowing URL ...")` and return `valid: true`. The blocklist + private-IP checks have already validated the hostname shape; a host that truly never resolves still fails at fetch time, where the webhook circuit breaker and durable-retry queue handle it gracefully. Test coverage in `tests/ssrf.test.ts` updated to match the new contract.
- **F6: usage records store `estimatedCost: null` + `costPricingMissing: true` for unknown Bedrock models** (was silent `$0`) — `server/routes/pipeline.ts`, `server/services/batch-scheduler.ts`, and the A/B test path in `server/routes/admin-content.ts` all branch on `estimateBedrockCost(...) === null` and write `null` + the flag instead of zero. `shared/schema.ts:usageRecordSchema` made `services.bedrock.estimatedCost` and `services.bedrockSecondary.estimatedCost` nullable. Frontend `formatCostPrecise` renders `—` for null and the spend table badges affected rows with `pricing?`. See the `estimateBedrockCost` gotcha above for the visibility model.
- **F7: `/api/calls/bulk-reanalyze` enforces a 24h sliding-window quota per admin** — `BULK_REANALYZE_DAILY_CAP` (default 200, env-tunable). Quota tracked in-process per instance via 1h-bucketed Map in `server/routes/utils.ts:checkAndRecordBulkReanalyzeQuota`; over-quota requests return 429 + `{ quota: { used, cap, retryAfterMs } }` before any AssemblyAI / Bedrock spend is incurred. Each in-flight bulk reanalyze runs the full pipeline (transcription + analysis), so an unmetered admin click could spend $5–$10 per batch without warning — the cap is the budget guardrail. In a multi-instance deploy each instance enforces its own cap; tighter accounting would need a Postgres-backed counter.
- **`alert: "score_distribution_alert"` log tag** — `server/services/auto-calibration.ts` emits `logger.warn` with this structured tag when the trailing-7d mean shifts ≥0.8 from the calibration window mean (default 30d). Severity `warning` at ≥0.8, `critical` at ≥1.5. Mirrors the synthetic-call `calibration_drift` pattern but for real calls — wires into CloudWatch metric filters the same way (operator action required; see Operator State Checklist). Fire-and-forget; failures log at `debug` and don't affect the calibration cycle.
- **`alert: "cost_budget_critical"` log tag** — `server/routes/admin-content.ts:GET /api/admin/cost-budget` emits this when MTD utilization ≥90% of `MONTHLY_COST_BUDGET_USD`. Wired to CloudWatch the same way as `score_distribution_alert` and `calibration_drift`. The endpoint also surfaces a `severity` field in the JSON response (`info` / `warning >75%` / `critical ≥90%` / `unknown` when budget unset); the spend-tracking page's `CostBudgetBanner` switches tone (sage / amber / destructive) accordingly.
- **`alert: "bedrock_access_denied"` log tag** — `server/services/bedrock.ts:reportBedrockAccessBlocked` emits `logger.warn` with this structured tag when AWS Bedrock returns 403 (Budget Action / SCP / IAM denial) or 429 (service quota throttling). Includes a `classification` field — `budget` / `quota` / `policy` / `other` — derived from the AWS error body so CloudWatch filters can split alerts by cause. Fires from BOTH `generateText` and `analyzeCallTranscript`. Mirrors the `score_distribution_alert` and `cost_budget_critical` patterns; the alert is parallel to the circuit breaker (which intentionally doesn't trip on these per INV-32) — operators get pushed alerts via CloudWatch and pull-side visibility via `/api/admin/health-deep`.
- **`subsystems.bedrockAI.accessBlocked24h` on `/api/admin/health-deep`** — total + per-classification breakdown of Bedrock 403/429 events in the last 24 hours, surfaced from `getBedrockAccessBlockedStats()` in `server/services/bedrock.ts`. In-process counter, hour-bucketed, prunes lazily on read. Promotes to an issue line when total ≥5/24h so small teams with occasional throttling don't get noise but sustained AWS-side blocking does. Companion to the `bedrock_access_denied` log tag — pull view for ops dashboards.
- **`ai_unavailable:bedrock_access_denied` analysis flag** — `server/routes/pipeline.ts` pushes this onto the analysis flag array when the AI-error catch detects a Bedrock 403 (`BedrockClientError.status === 403`) or a 429 message. Reviewers viewing the transcript see the flag in the existing flag-render UI (same pipeline as `output_anomaly:*` flags), so a call analyzed during a Budget Action window looks visibly different from a call where the AI just gave a generic verdict. Without this signal the empty rubric was indistinguishable from an under-confident AI output.
- **`S3Client.uploadFile` retries with backoff** — `server/services/s3.ts` retries transient S3 PUT failures up to 3 times with 200ms / 800ms / 3.2s exponential delays (~4s worst case before final throw). Idempotent uploads (same key + content) are safe to retry per AWS S3 semantics. Successful retry-after-failure logs at `info` level for retry-budget calibration. Scoped to `uploadFile` (binary audio) only — `putObject` direct callers (uploadJson, batch tracking) already have their own retry loops and double-retry would inflate worst-case latency without adding resilience. Post-incident driver: a batch-of-5 audio upload silently lost all 5 audio files because concurrent S3 PUTs hit transient throttling and the pipeline's silent-warn-and-continue masked the failure (transcription still worked because AssemblyAI got the buffer in-memory; users only noticed days later when opening a transcript page and seeing "Audio could not be loaded"). The retries close that race.
- **`alert: "s3_upload_failed"` log tag** — when `S3Client.uploadFile` exhausts all retries, it emits `logger.error` with this structured tag plus a `category` field (`audio` / `batch_inference` / `calibration` / `ab_test` / `usage` / `other`) derived from the object-key prefix. CloudWatch filters can split alerts by category — audio failures = user-facing missing-playback impact; batch_inference / calibration failures = cost-leak / observability impact. Counter feeds `/api/admin/health-deep`. Mirrors the `bedrock_access_denied` and `cost_budget_critical` patterns.
- **`subsystems.audioArchive.failures24h` and `subsystems.s3Uploads` on `/api/admin/health-deep`** — `getS3UploadFailureStats()` in `server/services/s3.ts` returns a 24h rolling count + per-category breakdown + last 10 failure keys. The admin endpoint surfaces TWO views: `audioArchive` (just the audio bucket, threshold 1 — every audio failure means a permanently missing playback file, real user impact) and `s3Uploads` (all categories, threshold 3 for non-audio — small calibration blips shouldn't page). Both downgrade overall health to `degraded` when triggered. In-process counter, hour-bucketed, prunes lazily on read.
- **SigV4 path encoding: encode once, share the encoded string between fetch URL and signer** — `server/services/s3.ts:request()` and `server/services/bedrock-batch.ts:s3Put/s3Get` both call `encodeCanonicalUri(path)` ONCE upstream and pass the resulting encoded string to BOTH the fetch URL AND `signRequest({ rawPath, pathAlreadyEncoded: true })`. The previous implementation passed the RAW path to both, relying on the WHATWG URL parser (used by fetch) and `encodeURIComponent` (used by `encodeCanonicalUri`) to produce the same encoding — but they don't agree on every character (WHATWG keeps `! ' ( ) *` literal; encodeURIComponent escapes them, and edge cases with literal spaces in 8x8 telephony filenames surfaced as `SignatureDoesNotMatch` rejections from S3). Encoding once eliminates the mismatch. The `pathAlreadyEncoded: true` flag on `signRequest` is the contract that says "I've encoded already, don't re-encode" — passing it is mandatory when you've pre-encoded, otherwise the signer would encodeURIComponent the `%20` sequences to `%2520`. Regression coverage in `tests/sigv4.test.ts:159-225`.
- **`scoringWeights` is server-validated to sum to ~100** — `shared/schema.ts:promptTemplateSchema` adds a `superRefine` rejecting weights whose four sub-weights don't total 100 ±2%. The frontend already shows a "total weight" indicator at template edit time; this closes the loop on writes (prompt-template create/update routes call into the schema). Existing in-DB templates that violate the constraint are unaffected on read; they fail validation only on the next edit. Strategic suggestion that landed: rubric weights are now structurally tunable end-to-end (schema → prompt builder injection → write-time validation).
- **`/api/coaching/:id/trajectory` is the per-agent post-coaching chart endpoint** — manager+ only. Returns up to N (default 20, max 50) scored points on each side of the session's `created_at`, each with overall score and the four sub-scores. Same exclusion semantics as `/outcome` (drops `excludedFromMetrics`). The frontend `TrajectorySection` component in `client/src/components/coaching/detail-panel.tsx` renders these as an SVG line chart with a coaching-boundary marker — managers can visually see "did this coaching move the needle?" rather than only seeing the single before/after delta. Hidden when there are fewer than 2 scored points.
...
- **Frontend Import-CSV button uses a hidden file picker, not an empty POST** — `client/src/pages/employees.tsx:391-419` wires both `Import CSV` buttons (toolbar + empty state) to a hidden `<input type="file" accept=".csv">`. Selecting a file fires `importMutation` which sends the file as `multipart/form-data` with `X-Requested-With: XMLHttpRequest` (matches the audio-upload pattern in `file-upload.tsx:96`). Prior implementation was POSTing an empty body and getting 400'd because the server route requires a `file` field per the A29 contract. Tooltip on the button documents the required column headers.
- **Employee CSV import accepts header aliases** — the per-row pick helper in `server/routes/employees.ts:152-175` reads the agent name from any of `Agent Name` / `Name` / `Full Name` / `Employee Name` (first non-empty wins). Same alias logic for `Department` / `Team` / `Group`, `Extension` / `Ext` / `Phone Extension`, and `Pseudonym` / `Display Name` / `Preferred Name` / `Nickname`. Status accepts `Status` or `Active`. Operators uploading CSVs from different HRIS exports no longer have to hand-edit headers to match a single canonical name. The original "Agent Name" header still works (it's just the first item in the alias list), so backward-compatibility is preserved.
- **Audio upload rate limit is env-tunable + frontend auto-retries 429** — `server/index.ts:417` reads `UPLOAD_RATE_LIMIT_PER_MIN` (default 30, was 10 before the batch-upload UX cycle). The frontend `file-upload.tsx:152-201` retries 429s up to 3 times with 5s/12s/25s backoff before surfacing the error to the user — a cold cumulative-window race typically clears within the first retry. Errored rows display a per-row `Retry` button + `uploadAll` includes errored files so "Upload all" after a partial failure picks up the failures alongside any newly-added pending files. Together this makes batch uploads of 10-20 calls (the normal manager workflow) reliable end-to-end without the prior dead-end where 429-failed rows had no UI path back to processing.
- **No DELETE /api/employees/:id route — deactivation is the pattern** — calls / coaching / snapshots / gamification all reference `employee_id`; hard-delete would orphan history. The intended lifecycle is `PATCH { status: "Inactive" }`. The employees-page row exposes a one-click toggle (X icon on Active rows, refresh icon on Inactive) via a dedicated `statusToggleMutation` separate from `updateMutation` so the success toast verbs match ("deactivated"/"reactivated") and the edit-dialog close side-effect doesn't fire when nobody opened it. Confirmation dialog explains the "preserves call history" semantics. If you ever do need a true hard-delete, it must cascade-clean every reference table — don't add a direct DELETE route.
...
|--------|-------|---------------|
| **Server Entry** | `server/index.ts`, `server/vite.ts`, `server/types.d.ts` | Express bootstrap. Middleware order: X-Forwarded-For validation → correlation ID (UUID-validated, truncated, falls back to randomUUID) → HTTPS redirect → CORS → `wafPreBody` → `express.json({limit:"1mb"})` → `express.urlencoded({limit:"1mb"})` → `wafPostBody` → security headers → audit logging → CSRF double-submit (timingSafeEqual on hashed tokens) → legacy CSRF Content-Type check → routes → `globalErrorHandler` (AppError-aware, transitional `{message, error:{...}}` shape, prod 5xx sanitization). Env validation, graceful shutdown, Vite dev server integration. Conditional dynamic import of `./test-mocks/setup` at module load when `E2E_MOCKS=true`. `types.d.ts` holds Express.User and SessionData type augmentations. |
| **Test Mocks** | `server/test-mocks/handlers.ts`, `server/test-mocks/setup.ts` | MSW-Node handlers + lifecycle. Activated when `E2E_MOCKS=true` via the conditional import in `server/index.ts`. Intercepts AssemblyAI (upload + submit + poll), Bedrock (Converse + Titan embed), S3 PUT/GET, and IMDS so Playwright e2e tests can drive the full audio pipeline without live credentials. `TEST_FIXTURES` export gives test code deterministic access to the mock transcript ID / URL / text / analysis. `onUnhandledRequest: "bypass"` preserves localhost traffic. Zero production path — `msw` is a devDependency. |
...
| **Reports** | `server/routes/reports.ts` | Search, agent profiles, filtered reports, AI-generated agent summaries, scoring-corrections self-view (`GET /api/scoring-corrections/mine`), semantic search (`GET /api/search/semantic` — pgvector cosine when available + bulk hydration via `storage.getCallsWithDetailsByIds` for O(log N + K), in-memory fallback, hybrid mode `?mode=hybrid&alpha=N` fusing keyword reciprocal-rank + cosine, relevance threshold `?threshold=0.25` default, layered `from`/`to`/`sentiment`/`employeeId` filters, 200-entry LRU query-embedding cache, graceful keyword fallback), server-side CSV + PDF exports (`GET /api/reports/filtered/export.{csv,pdf}`, `GET /api/reports/agent-profile/:employeeId/export.{csv,pdf}` — all emit `event: "export_report"` audit entries; filtered-report PDF embeds a monthly-trend line chart via the shared PDF chart primitives). The legacy `/api/reports/export-beacon` route + client-built TXT/CSV downloads were retired — all exports now go through the server-side endpoints. |
| **Coaching** | `server/routes/coaching.ts` | Coaching session CRUD, action item toggling, webhook triggers, per-session outcome (`GET /api/coaching/:id/outcome`), program-wide outcomes aggregate (`GET /api/coaching/outcomes-summary` — `?groupBy=manager\|employee` per-group breakdown, `?bucket=week` weekly time-series, `?includeSkipped=true` drill-in, `avgSubDeltas` sub-score deltas on every rollup) — powered by SQL-native `storage.getCoachingOutcomes()` on Postgres. CSV + PDF exports (`/export.csv`, `/export.pdf` — both emit `event: "export_report"` audit entries) share a `computeManagerOutcomeRows(windowDays)` helper. The PDF export embeds a per-manager bar chart via the shared PDF chart primitives. |
| **Users** | `server/routes/users.ts` | User management (admin CRUD, password reset/change, MFA). `GET /api/users/unlinked` returns viewer users with no matching employee row — backs the admin Users-tab "Onboarding" banner. `POST /api/users/:id/link-employee` reconciles a user to an employee by updating the user's displayName; audit event `user_employee_link_created`. |
...
| **Gamification** | `server/routes/gamification.ts` | Leaderboard, badges, points, stats. Server-side CSV export via `GET /api/gamification/leaderboard/export.csv` (manager+, MFA-gated, emits `event: "export_report"` audit entry). |
...
| **Login return-to guard** | `client/src/lib/return-to.ts` | SSRF-guarded parser for `?return_to=<url>` on the login page. Accepts only http(s) URLs whose hostname is `umscallanalyzer.com` or a subdomain; rejects open-redirect lookalikes (`umscallanalyzer.com.evil.com`, `fakeumscallanalyzer.com`, `javascript:`/`data:`). Called by `pages/auth.tsx` after login success AND by `AuthenticatedApp` on mount when an already-logged-in user hits `/auth?return_to=…` — needed because CA's React router has no `/auth` route (only AuthPage for logged-out users), so without the mount-time redirect the user lands on the NotFound page. |
| **Security** | `server/services/audit-log.ts`, `server/services/security-monitor.ts`, `server/services/vulnerability-scanner.ts`, `server/services/incident-response.ts` | HIPAA audit logging (dual-write, HMAC chain, persistent integrity head), brute-force/credential stuffing detection (wired to client IP via `passReqToCallback`), automated vuln scanning (history retains hollow entries past cap, summary kept), incident lifecycle management (DB-first persist; randomUUID IDs; throws on persist failure). F-06: `startIntegrityPersistScheduler()` / `stopIntegrityPersistScheduler()` — 30s periodic persist of the in-memory HMAC chain head to `audit_log_integrity`, skipping the DB write when the head hasn't advanced since last successful persist. Bounds the crash-mid-burst gap between fire-and-forget per-entry persists and the on-disk head. Timer `.unref()`'d per INV-30; wired into startup right after `loadAuditIntegrityChain` and into graceful shutdown BEFORE the final awaited `persistIntegrityChainHead()` call to avoid a late interval-triggered write racing the DB pool close. |
...
| **Resilience** | `server/services/resilience.ts` | Circuit breaker (5 failures → 30s open → half-open test) wrapping Bedrock calls. Consumed by `bedrock.ts` (on-demand) and `bedrock-batch.ts` (batch — F-18) which share the same breaker instance — an outage detected on either path protects both. `execute()` takes an optional `isFailure(err)` predicate; Bedrock uses it to keep `BedrockClientError` (4xx, except 429) from tripping the breaker (F-17/F-18) — only 5xx + 429 count toward the open threshold. **Exports typed `CircuitBreakerOpenError` (Tier C #10/F5)** — `execute()` throws this instead of a generic Error when rejecting due to open state. Callers can `instanceof`-check it to cleanly detect "rejected by policy" vs. "fn threw"; `webhooks.ts` retry loop uses this to abandon in-process retries when the breaker trips mid-sequence, closing a TOCTTOU race between a pre-loop `isOpen()` check and `execute()`. Phase C: also exports `PerKeyCircuitBreaker` — keyed variant with independent per-key state machines and an LRU-bounded (1k) key map. Used by `webhooks.ts` for per-webhook-URL isolation so one broken receiver doesn't brownout the rest. `PerKeyCircuitBreaker.execute(key, fn, options)` accepts an options object `{ isFailure?, threshold?, resetMs? }` — per-key threshold/resetMs overrides are applied on first breaker creation per key (subsequent policy changes require an explicit `reset(key)` to recreate). |
| **Job Queue** | `server/services/job-queue.ts` | PostgreSQL-backed durable queue with `FOR UPDATE SKIP LOCKED`, 30s worker heartbeat, 2min stale reap, attempts-on-failJob contract (A18). `getJob(jobId)` lookup added (A8) for polling-based async job patterns. `enqueue(type, payload, {delayMs})` defers first pickup via `locked_at = NOW() + interval`; used by the webhook persistent-retry handoff for a 60s initial backoff. Worker handlers in `routes.ts`: `process_audio` (audio pipeline), `batch_snapshots` (snapshot generation), `deliver_webhook` (persistent webhook retry — handoff from in-process retry exhaustion). |
| **WebSocket** | `server/services/websocket.ts` | Authenticated WebSocket server broadcasting real-time call processing status |
| **Webhooks** | `server/services/webhooks.ts` | HMAC-signed HTTP POST notifications on call events with retry logic and SSRF validation. Exports `setWebhookRetryEnqueuer(fn)` + `redeliverWebhook({webhookId, event, body, previousAttempts})` so the in-process `deliverWithRetry` can hand off to the durable PostgreSQL job queue after 4 in-process retries exhaust — survives process restart. Without `DATABASE_URL`, the enqueuer is never wired and behavior falls back to prior fire-and-forget. Phase C: per-webhook circuit breaker (`webhookBreaker` — default 5 failures / 5 min threshold, LRU-bounded at 1k keys) wrapping `deliverWebhook`. Open circuit skips BOTH delivery AND durable-retry enqueue for that `webhookId`. Exports `getWebhookBreakerSnapshot` for the admin observability endpoint. Powers the `/admin/webhooks-health` dashboard. Per-webhook `retryPolicy` overrides (`maxInProcessRetries`, `circuitThreshold`, `circuitResetMs`) consumed by `deliverWithRetry` and the per-key breaker; PATCH on `/api/admin/webhooks/:id` resets the breaker when policy changes so new thresholds take effect immediately. F-02: `WebhookHttpError` class + `isWebhookCircuitFailure` predicate — `deliverWebhook` throws `WebhookHttpError` carrying the numeric HTTP status; `deliverWithRetry` always passes `{ isFailure: isWebhookCircuitFailure, ... }` to `webhookBreaker.execute()` so 4xx (except 429) responses do NOT count toward the breaker's open threshold. Mirrors Bedrock's INV-32 treatment — a misconfigured webhook URL returning 404 still fails each delivery but doesn't also block the durable-retry enqueue for 5 minutes. Network errors, timeouts, 429, and 5xx still count. |
| **Coaching Alerts** | `server/services/coaching-alerts.ts` | Auto-creates coaching sessions for low/high-score calls, detects recurring weaknesses. Tier C #11/F6: `checkRecurringWeaknesses` has a module-level in-flight `Map<employeeId, timestamp>` with 5-min TTL so two concurrent pipeline completions for the same agent don't both pass the spam guard and create duplicate plans. Finally-block clears on success OR failure; TTL is the safety net for a crashed process. |
| **Agent Decline Alerts** | `server/services/agent-decline-alert.ts` | Scheduler that periodically computes per-employee health-pulse (current-vs-prior window comparison, same math as `/api/analytics/health-pulse/:employeeId`) and fires webhook event `agent.decline_alert` when overall delta ≤ `-AGENT_DECLINE_THRESHOLD`. Off by default via env flag. Tier A #2: **dedup is DB-backed when `DATABASE_URL` is set** — `agent_decline_alert_history` table persists `(employee_id, last_alerted_at)` and the next cycle loads employeeIds alerted within a cool-off window (hardcoded 2× the cycle interval, so 48h with the default 24h cadence). A process restart no longer re-fires alerts for currently-declining agents. In-memory `previouslyAlerted` set remains as fallback when DB is unavailable. Log line carries `dedupBackend: "db" | "in-memory"`. Opportunistic prune of expired history rows runs at each cycle start. Timers `.unref()`'d per INV-30; wired into graceful-shutdown sequence. |
| **Calibration Assertions** | `server/services/calibration-assertions.ts` | Post-analysis fire-and-forget hook (Tier C #9) for synthetic calls: looks up the originating simulated-call preset via `findSimulatedCallBySentToAnalysisCallId`, compares the computed performance score against the preset's `expectedScoreRange`, and emits `logger.warn` with a structured `alert: "calibration_drift"` tag when out of range. CloudWatch metric filters can match the tag so prompt-template / model regressions surface in existing alerting stacks automatically. Called from `pipeline.ts` via dynamic import in the synthetic-call branch; non-synthetic calls skip. Failures in the assertion itself log at `debug`. |
...
| **Frontend Entry** | `client/src/main.tsx`, `client/src/App.tsx` | React SPA root: auth gate, 25 lazy-loaded pages, WebSocket connection, idle timeout, keyboard shortcuts. `App.tsx` also owns the mobile-drawer `sidebarOpen` state and the `lg:hidden` fixed-position hamburger button (44×44 tap target, top-left). Sidebar auto-closes on route change via `useEffect` on `location`. |
...
| **Frontend Pages** | `client/src/pages/` | 29+ page components (dashboard, upload, transcripts, reports, coaching, my-coaching, admin [Users CRUD + Access Requests + Role Definitions tabs], system-health [runtime signals + `PipelineTrendsCard` showing today-vs-7-day deltas for call count, failure rate, avg AI duration, avg cost-per-call, plus a 7-day cost sparkline], batch-status [admin batch inference dashboard, auto-refresh 30s], leaderboard, `/admin/soft-fail` [soft-fail-status.tsx — admin-only live observer of the Operator State Checklist, 30s auto-refresh; surfaces 12 silent-degradation checks with per-item status + fix hints], etc.) |
...
**Live observer**: a runtime view of most of the items below is available at **`/admin/soft-fail`** (admin-only, 30s auto-refresh) via `GET /api/admin/soft-fail-status`. The page surfaces 12 of these conditions with current status (ok / warning / error / unknown), a plain-English message, and a per-item fix hint. This checklist remains the authoritative spec; the page is the observer. Visit it on first login to a fresh environment.
...
- [ ] `BEDROCK_MODEL` must be in `BEDROCK_PRICING` if set — `server/index.ts` startup and `server/routes/utils.ts:warnOnUnknownBedrockModel`. Without this, cost tracking stores `null` + `costPricingMissing: true` (F6) and the spend dashboard badges affected rows with `pricing?`; total roll-ups underreport the model side until the operator updates `BEDROCK_PRICING`. **LOW risk** — visible badge + boot warn + once-per-model runtime warn (was **HIGH** before F6 made the gap observable; was silent `$0` before that).
- [ ] **Viewer user accounts must be linkable to an employee row** — `server/auth.ts:getUserEmployeeId()` matches `username→email` then `displayName→name`. If no match, viewers see empty call lists and 403 on agent endpoints. Silent degradation — no startup warning. **MEDIUM risk** — viewer appears authenticated but sees no data. Now observable AND directly actionable: `GET /api/users/unlinked` returns the list of viewer accounts with no matching employee, the admin Users tab renders an amber-stripe "Onboarding" banner listing them, AND each row in the banner has an inline "Link to employee…" dropdown + button that calls `POST /api/users/:id/link-employee` to set the user's displayName to match a chosen employee — single-click reconciliation. Ensure DB employee records use the same email as the corresponding user's login; or click "Link" to resolve via displayName.
...
- [ ] **Semantic search depends on populated `call_analyses.embedding`** — the pipeline computes embeddings fire-and-forget during analysis (`server/routes/pipeline.ts:generateCallEmbedding`). Historical calls analyzed before embeddings were wired, or whose embedding fetch failed transiently, won't appear in `/api/search/semantic` results. The response exposes the gap via `coverage: { totalAccessible, withEmbeddings }`. Without Bedrock embedding access, the endpoint silently falls back to keyword search (`mode: "keyword-fallback"`). **LOW risk** — graceful degradation in place; if admin wants full semantic reach over historical data, bulk-reanalyze calls to populate the embedding column.
- [ ] **pgvector extension installed on the PostgreSQL server** — auto-attempted at startup via `CREATE EXTENSION IF NOT EXISTS vector` in `server/db/pool.ts:runMigrations`. Fails silently if the DB doesn't support pgvector (e.g., RDS PostgreSQL <15.2) — `isPgvectorAvailable()` stays false, `updateCallAnalysis` skips the embedding_vec dual-write, and `/api/search/semantic` falls back to O(N) in-memory scoring. Verify via startup log: `pgvector extension enabled — native vector similarity search available`. **MEDIUM risk for scale** — in-memory scoring scales linearly with call count; pgvector is required for tenants with >10k calls to avoid memory pressure. **LOW risk** at smaller scale.
- [ ] **Run `npm run backfill-embeddings` once after pgvector is first enabled** — pipeline populates `embedding_vec` for NEW calls going forward, but historical `call_analyses` rows keep only the JSONB `embedding` column until the backfill runs. Until the script completes, `/api/search/semantic`'s pgvector fast path will report low coverage (visible in response `coverage` field AND the UI's amber "coverage N/M · low" warning). Idempotent; batches at 100 rows/pass. **LOW risk** — the in-memory fallback can still score JSONB embeddings directly during the transition, so no results are lost.
- [ ] **Semantic search `?threshold=0.25` default prunes weak matches** — API callers expecting unlimited top-N from `/api/search/semantic` must pass `?threshold=0` explicitly. The bundled UI uses the default. A caller that previously scraped the endpoint and got 20 results will now get fewer when tail matches fall below 0.25 cosine; behavior is intentional (low-similarity padding was noise). **LOW risk** — API contract change only affects consumers outside the bundled UI.
- [ ] **Persistent webhook retry requires `DATABASE_URL`** — without a DB-backed job queue, `setWebhookRetryEnqueuer` is never wired and webhook exhaustion stays a `logger.error` with no durable retry. Prior fire-and-forget behavior is preserved, so this is a feature-availability gap, not a correctness bug. **LOW risk** — opt-in durability for webhook delivery; absent DB, the 4 in-process retries are the full retry budget.
- [ ] **Webhook subscription to `agent.decline_alert` event** (Tier 2 #6) — when `AGENT_DECLINE_CHECK_ENABLED=true`, the scheduler fires this event every cycle for qualifying agents. Without a webhook config whose `events` array includes this event, alerts fire on the server but no external delivery happens. **MEDIUM risk** — feature-gap rather than data loss; operators who turn the flag on expecting notifications need to pair it with a webhook subscription.
- [ ] **Simulated-call presets need `config.expectedScoreRange` for calibration-suite coverage** — `npm run seed` attaches tier-based defaults on fresh seeds (excellent 8-10, acceptable 5.5-7.5, poor 2-4.5). Presets seeded before the Tier 1 cycle don't have it; admins can add ranges via direct DB `UPDATE simulated_calls SET config = jsonb_set(config, '{expectedScoreRange}', ...)` or by deleting + re-seeding. **LOW risk** — cosmetic / feature-completeness; without ranges the calibration-suite UI shows "No calibration presets configured".
- [ ] **(Optional) Wire CloudWatch metric filter on `alert: "calibration_drift"` log lines** — Tier C #9 emits this structured log tag when a synthetic call's analyzed score lands outside its preset's `expectedScoreRange`. Without a matching metric filter + alarm, the regression signal only appears in log lines (no external notification). **MEDIUM risk** — feature-gap rather than data loss; operators running the calibration suite expecting automatic alerts on prompt/model regressions need to wire this CloudWatch filter.
- [ ] **`agent_decline_alert_history` table auto-created on next boot** — Tier A #2 adds this table via idempotent `CREATE TABLE IF NOT EXISTS` in `server/db/pool.ts:runMigrations`. No manual step or backfill needed. Verify the table exists after deploy if the decline-alerts feature is enabled (`AGENT_DECLINE_CHECK_ENABLED=true`) so dedup survives restarts. **LOW risk** — migration handles it, but silent if DB connectivity drops during runMigrations.
- [ ] **`MONTHLY_COST_BUDGET_USD` env var (optional)** — without it, `/api/admin/cost-budget` returns `monthlyBudget: null`, the `CostBudgetBanner` on `/admin/spend` shows "monthly budget unset" and never gates utilization tracking, and `alert: "cost_budget_critical"` never fires. **LOW risk** — observability gap; operators tracking spend should set this to enable the banner severity bands and the critical alarm.
- [ ] **(Optional) Wire CloudWatch metric filter on `alert: "score_distribution_alert"` log lines** — `server/services/auto-calibration.ts` emits this when the trailing-7d call-score mean shifts ≥0.8 from the calibration window mean (severity warning ≥0.8, critical ≥1.5). Without a matching filter + alarm, the regression signal only appears in pm2 logs (no operator paging). **MEDIUM risk** — feature-gap; mirrors the existing `calibration_drift` pattern (which is also unwired by default, so consider wiring both at once).
- [ ] **(Optional) Wire CloudWatch metric filter on `alert: "cost_budget_critical"` log lines** — `server/routes/admin-content.ts:GET /api/admin/cost-budget` emits this when MTD utilization ≥90% of `MONTHLY_COST_BUDGET_USD`. Without the filter, budget overshoots only show in logs. **MEDIUM risk** — feature-gap. Useless until `MONTHLY_COST_BUDGET_USD` is also set.
- [ ] **(Optional) Wire CloudWatch metric filter on `alert: "bedrock_access_denied"` log lines** — `server/services/bedrock.ts` emits this tag with a `classification` field (`budget` / `quota` / `policy` / `other`) when AWS Bedrock returns 403 or 429. Without a matching filter + alarm, AWS Budget Actions and service quotas silently degrade analysis quality (calls complete with the `ai_unavailable:bedrock_access_denied` flag instead of a real rubric). **MEDIUM risk** — feature-gap; mirrors the existing `calibration_drift`, `score_distribution_alert`, and `cost_budget_critical` patterns. Pair the filter with an SNS alarm and consider routing budget-classified events to your finance channel and quota-classified events to ops.
- [ ] **(Optional) Wire CloudWatch metric filter on `alert: "s3_upload_failed"` log lines** — `server/services/s3.ts:S3Client.uploadFile` emits this tag with a `category` field (`audio` / `batch_inference` / `calibration` / `ab_test` / `usage` / `other`) when an S3 PUT exhausts the 3-retry budget. The category split is operationally important: `category=audio` failures cause permanent user-visible missing-playback issues (each one needs a re-upload to recover); `category=batch_inference` failures risk orphaned AWS Bedrock batch jobs (cost leak). Without the filter, both surface only in pm2 logs + the `/admin/health-deep` pull view. **MEDIUM risk** — feature-gap; mirrors `calibration_drift`, `score_distribution_alert`, `cost_budget_critical`, `bedrock_access_denied`. Pair with an SNS alarm and consider routing audio-category events to ops (user-facing) and batch_inference-category events to a finance/cost channel.
- [ ] **`BULK_REANALYZE_DAILY_CAP` env var (optional, default 200)** — per-admin 24h sliding cap on `/api/calls/bulk-reanalyze`. Default is sane; documented here so operators tuning for high-volume teams (or tightening for cost discipline) know the knob exists. In multi-instance deploys each instance enforces its own cap; tighter accounting would need a Postgres-backed counter. **LOW risk** — admins hit 429 with `{ quota: { used, cap, retryAfterMs } }` after 200 calls/24h.
...
_No pending items. Previously listed: `/api/admin/jobs/:id` MFA bypass — fixed (see Common Gotchas and Operator State Checklist above). Add new items here when an audit surfaces a gap that will be addressed in the next cycle._
...
Core Architecture & Pipeline:
  server/index.ts, server/routes.ts, server/routes/pipeline.ts, server/routes/utils.ts, server/routes/config.ts, server/routes/admin.ts, server/middleware/waf.ts, server/middleware/rate-limit.ts, server/middleware/error-handler.ts, server/types.d.ts, server/vite.ts, server/constants.ts, server/services/job-queue.ts, server/services/logger.ts, server/services/correlation-id.ts, server/services/tracing.ts, server/services/trace-span.ts, server/services/sentry.ts, server/services/websocket.ts, server/services/pipeline-settings.ts, server/test-mocks/handlers.ts, server/test-mocks/setup.ts
...
RAG & Knowledge Base:
  server/services/rag-client.ts, server/services/rag-sso-client.ts, server/services/best-practice-ingest.ts, server/services/medical-synonyms.ts, server/services/scoring-feedback.ts
...
INV-29 | gracefulShutdown must call all seven scheduler stop functions (batch, calibration, telephony, reports, transcribing-reaper, agent-decline-alert, integrity-persist) with independent try/catch before jobQueue.stop() | Subsystem: Core Architecture
