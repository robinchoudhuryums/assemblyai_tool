...
| **Resilience** | `server/services/resilience.ts` | Circuit breaker (5 failures → 30s open → half-open test) wrapping Bedrock calls (single consumer: `bedrock.ts`). `execute()` takes an optional `isFailure(err)` predicate; Bedrock uses it to keep `BedrockClientError` (4xx) from tripping the breaker (F-17) — only 5xx + 429 count toward the open threshold. |
