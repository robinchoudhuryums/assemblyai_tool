...
AGENT_DECLINE_CHECK_INTERVAL_HOURS # How often to scan active employees (default: 24, range: 1-168)
...
- **Agent decline scheduler is off by default** (Tier 2 #6) — `AGENT_DECLINE_CHECK_ENABLED=true` enables the scheduler. When disabled, `startAgentDeclineScheduler()` logs "scheduler disabled" and returns a no-op stopper. In-memory dedup is lost on process restart; a just-restarted process may re-fire one alert per currently-declining agent on first cycle (acceptable given the default 24h cadence). The `agent.decline_alert` webhook event only delivers if at least one webhook config subscribes to it — enabling the scheduler without a subscriber produces internal log lines but no external notification.
...
| **Agent Decline Alerts** | `server/services/agent-decline-alert.ts` | Scheduler that periodically computes per-employee health-pulse (current-vs-prior window comparison, same math as `/api/analytics/health-pulse/:employeeId`) and fires webhook event `agent.decline_alert` when overall delta ≤ `-AGENT_DECLINE_THRESHOLD`. Off by default via env flag. In-memory dedup prevents consecutive-cycle re-alerts for the same declining agent; dedup set rebuilds each cycle so a stabilized-then-declining agent re-alerts. Timers `.unref()`'d per INV-30; wired into graceful-shutdown sequence. |
...
Engagement & Reporting:
  server/services/gamification.ts, server/services/coaching-alerts.ts, server/services/performance-snapshots.ts, server/services/scheduled-reports.ts, server/services/simulated-call-storage.ts, server/services/agent-decline-alert.ts, server/routes/coaching.ts, server/routes/gamification.ts, server/routes/analytics.ts, server/routes/reports.ts, server/routes/insights.ts, server/routes/snapshots.ts, server/routes/dashboard.ts, server/routes/employees.ts, server/routes/calls.ts, server/routes/calls-tags.ts, server/routes/admin-operations.ts, server/routes/admin-content.ts, server/routes/simulated-calls.ts
Frontend / UI:
