...
import { recordDataAccess, getRecentAlerts, acknowledgeAlert, getSecuritySummary, createBreachReport, updateBreachStatus, getAllBreachReports } from "./services/security-monitor";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/auth/mfa/users", requireAuth, requireRole("admin"), async (_req, res) => {
    try {
  ...
  app.get("/api/admin/security-summary", requireAuth, requireRole("admin"), (_req, res) => {
    res.json(getSecuritySummary());
  });

  // Recent security alerts
  app.get("/api/admin/security-alerts", requireAuth, requireRole("admin"), (_req, res) => {
    res.json(getRecentAlerts());
  });

  // Acknowledge a security alert
  app.patch("/api/admin/security-alerts/:id", requireAuth, requireRole("admin"), (req, res) => {
  ...
  // Breach reports
  app.get("/api/admin/breach-reports", requireAuth, requireRole("admin"), async (_req, res) => {
    try {
  ...
  app.post("/api/admin/breach-reports", requireAuth, requireRole("admin"), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/breach-reports/:id", requireAuth, requireRole("admin"), async (req, res) => {
    try {
...
async function processAudioFile(callId: string, filePath: string, audioBuffer: Buffer, originalName: string, mimeType: string, callCategory?: string, uploadedBy?: string, processingMode?: string) {
  ...
  // Get tags for a call
  app.get("/api/calls/:id/tags", requireAuth, async (req, res) => {
    try {
      const callId = req.params.id;
      const pool = getPool();
      if (pool) {
  ...
  // Add a tag to a call
  app.post("/api/calls/:id/tags", requireAuth, async (req, res) => {
    try {
      const callId = req.params.id;
      const { tag } = req.body;
  ...
  app.delete("/api/calls/:id/tags/:tagId", requireAuth, async (req, res) => {
    try {
      const pool = getPool();
  ...
  // Search calls by tag
  app.get("/api/calls/by-tag/:tag", requireAuth, async (req, res) => {
    try {
      const pool = getPool();
      if (!pool) return res.json([]);
  ...
  app.get("/api/analytics/team/:teamName", requireAuth, async (req, res) => {
    try {
      const pool = getPool();
      if (!pool) return res.json([]);
  ...
  // Export calls as CSV
  app.get("/api/export/calls", requireAuth, requireRole("manager", "admin"), async (req, res) => {
    try {
      const pool = getPool();
      const dateFrom = req.query.from as string | undefined;
      const dateTo = req.query.to as string | undefined;
  ...
  app.get("/api/export/team-analytics", requireAuth, requireRole("manager", "admin"), async (req, res) => {
    try {
      const pool = getPool();
