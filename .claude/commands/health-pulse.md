...
Are there any known or suspected cases of silent degradation — where the app starts and reports healthy but is operating with broken or missing state? (Examples: a subsystem that logs a warning at boot and continues, a health endpoint that returns 200 while a background service is disabled, a feature that silently falls back to a no-op when a dependency is absent.) List any suspected cases even if low confidence.
